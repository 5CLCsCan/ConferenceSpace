...
import { Loader2, Save, X, User as UserIcon, Mail, Shield } from "lucide-react"
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="space-y-6">
              ...
              <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <h2 className="text-2xl font-semibold leading-tight">{fullName}</h2>
                ...
                {displayRoles.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {displayRoles.map((role) => (
                      <Badge key={role} variant="secondary" className="flex items-center gap-1 border border-primary/30 bg-white/80 px-3 py-1 text-xs uppercase tracking-wide">
          ...
          <div className="grid gap-6 lg:grid-cols-12">
            <Card className="lg:col-span-7">
              <CardHeader className="border-b pb-4">
                <CardTitle className="text-lg font-semibold">{t("profile.sections.personalInfo")}</CardTitle>
                <CardDescription>{t("profile.sections.personalInfoDescription")}</CardDescription>
              </CardHeader>
            ...
            <Card className="lg:col-span-5">
              <CardHeader className="border-b pb-4">
                <CardTitle className="text-lg font-semibold">{t("profile.sections.expertise")}</CardTitle>
                <CardDescription>{t("profile.sections.expertiseDescription")}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6 pt-6">
                <div className="space-y-2">
                ...
                {formData.domain.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {formData.domain.map((domain, index) => (
                      <Badge key={index} variant="secondary" className="gap-1 bg-primary/10 pr-1 text-primary">
                        {domain}
                        <button
                          type="button"
                  ...
                  <p className="text-sm text-muted-foreground">{t("profile.highlights.domainsHint")}</p>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          {isDirty && (
            <div className="sticky bottom-4 z-10 flex flex-col items-stretch gap-3 rounded-2xl border border-primary/20 bg-white/90 px-6 py-4 shadow-sm backdrop-blur lg:flex-row lg:justify-end">
              <Button
                variant="outline"
