...
export function ConferenceCommittee({ conferenceId, className }: ConferenceCommitteeProps) {
  ...
  const committeeMembers = useMemo((): CommitteeMember[] => {
    ...
    return members
  }, [conference, conferenceReviewers, pendingInvitations, resolvedUsers])
                  ...
                  {selectedUsers.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {selectedUsers.map((user) => (
                        <span
                          key={user.email}
                          className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded-full bg-[#1B3C53]/10 text-[#1B3C53]"
                        >
                          {user.email}
                          <button
                            type="button"
