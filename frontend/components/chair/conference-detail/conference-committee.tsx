...
export function ConferenceCommittee({ conferenceId, className }: ConferenceCommitteeProps) {
  ...
  return (
    <div className={cn("space-y-5", className)}>
      {loading ? (
        <div className="text-xs text-slate-500">{t("runtime.components.chair.conference-detail.conference-committee.text_loading_committee")}</div>
      ) : error ? (
        ...
        <>
          ...
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <h3 className="text-sm font-bold text-[#1B3C53] tracking-tight">{t("runtime.components.chair.conference-detail.conference-committee.text_committee_members")}</h3>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 text-[10px] uppercase text-slate-400 font-bold border-b border-slate-200 tracking-widest">
                  <tr>
                    <th className="px-4 py-2.5">{t("runtime.components.chair.conference-detail.conference-committee.text_name")}</th>
                    <th className="px-4 py-2.5">{t("runtime.components.chair.conference-detail.conference-committee.text_email")}</th>
                    <th className="px-4 py-2.5">{t("runtime.components.chair.conference-detail.conference-committee.text_role")}</th>
                    <th className="px-4 py-2.5">{t("runtime.components.chair.conference-detail.conference-committee.text_domains")}</th>
                  </tr>
                </thead>
                  ...
                  {members.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="px-4 py-6 text-xs text-slate-500 text-center">
                        {t("runtime.components.chair.conference-detail.conference-committee.text_no_committee_members_found")}{" "}</td>
                    </tr>
                  ) : (
                    members.map((member) => (
                      <tr key={member.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-600">{t("runtime.components.chair.conference-detail.conference-committee.text_reviewer")}</td>
