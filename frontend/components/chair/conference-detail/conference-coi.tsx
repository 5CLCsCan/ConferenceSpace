...
export function ConferenceCOI({ conferenceId, className }: ConferenceCOIProps) {
  ...
  const headerMessage = useMemo(
    () =>
      t("runtime.components.chair.conference-detail.conference-coi.text_this_view_is_fully_api_backed") +
      " /api/v1/coi/* " +
      t(
  ...
  return (
    <div className={cn("space-y-6", className)}>
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-[#1B3C53] tracking-tight">
            {t("runtime.components.chair.conference-detail.conference-coi.text_conflicts_of_interest_management")}
          </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard label="Total Conflicts" value={stats.totalConflicts} icon="list_alt" iconBgClass="bg-slate-50" iconTextClass="text-[#1B3C53]" />
        <StatCard label="Pending Review" value={stats.pendingReview} icon="pending_actions" iconBgClass="bg-yellow-50" iconTextClass="text-yellow-700" />
        <StatCard label="Auto-Detected" value={stats.autoDetected} icon="smart_toy" iconBgClass="bg-blue-50" iconTextClass="text-blue-700" />
        <StatCard label="Orphaned Papers" value={stats.orphanedPapers} icon="warning" iconBgClass="bg-red-50" iconTextClass="text-red-700" />
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1 bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="rounded-md border-x-0 border-t-0 border-b border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            {t("runtime.components.chair.conference-detail.conference-coi.text_confirm_dismiss_and_reviewer_reassignment_coi")}
          </div>

          {loading ? (
            <div className="p-6 text-xs text-slate-500">
              {t("runtime.components.chair.conference-detail.conference-coi.text_loading_coi_relationships")}
            </div>
          ) : error ? (
                  ...
                  {relationships.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-3 py-8 text-center text-xs text-slate-500">
                        {t("runtime.components.chair.conference-detail.conference-coi.text_no_coi_relationships_found")}
                      </td>
                    </tr>
                  ) : (
                    relationships.map((relationship) => (
                      <tr key={relationship.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] text-slate-500">{relationship.reviewer_email}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-3">
                          <span className={cn("inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold border uppercase tracking-wider", getSeverityClasses(relationship.severity))}>
                            {relationship.type}
                          </span>
                        </td>
                        <td className="px-3 py-3">
          ...
          {!loading && !error && total > 0 && (
            <div className="px-4 py-3 border-t border-slate-200 flex items-center justify-between">
              <div className="text-[11px] text-slate-500">
                ...
                -<span className="font-bold text-[#1B3C53]">{Math.min(currentPage * PAGE_SIZE, total)}</span>{" "}
