...
import { AIAssistantCard, AbstractCard } from "../review-sidebar"
    ...
    briefing: {
      ...
      artifact: {
        ...
        guardrails: {
          ...
          no_score: true,
          bias_notice:
            "This briefing is assistive only and must not replace independent review judgment.",
        },
      },
    },
    loading: false,
...
describe("AIAssistantCard", () => {
  it("uses semantic aliases for the abstract shell", () => {
    render(
      <AbstractCard
        submission={{
          id: "42",
          ...
          title: "Reliable Systems",
          abstract: "Structured reviewer pre-read workflow.",
          keywords: ["review", "workflow"],
          track: "main",
          status: "under_review",
          dueDate: "",
          daysLeft: 0,
          conference: { id: "1", acronym: "CONF", name: "Conference" },
        }}
      />,
    )

    expect(screen.getByText(/abstract/i)).toHaveClass("text-card-header")
    expect(screen.getByText(/structured reviewer pre-read workflow/i)).toHaveClass("text-body")
    expect(screen.getByText("review")).toHaveClass("badge-neutral", "text-ui-meta")
  })
  ...
  it("renders a ready-state card and opens the modal analysis view on demand", () => {
    ...
    expect(screen.getByText(/ai assistant/i)).toHaveClass("text-table-header")
    ...
    expect(screen.getByText(/review readiness signals/i)).toHaveClass("text-kicker")
