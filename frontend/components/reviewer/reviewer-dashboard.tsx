  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64 text-slate-400 gap-2">
        ...
        {t("runtime.components.reviewer.reviewer-dashboard.text_loading_reviewer_dashboard")}{" "}</div>
    )
  }
  ...
  if (error) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 text-red-700 px-4 py-3 text-sm">
        {t("runtime.components.reviewer.reviewer-dashboard.text_failed_to_load_reviewer_dashboard")}{" "}{error}
      </div>
    )
  }
    ...
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-[32px] font-bold tracking-tight text-[#1B3C53] dark:text-white">
          {t("runtime.components.reviewer.reviewer-dashboard.text_reviewer_dashboard")}{" "}</h1>
        <p className="text-sm font-light leading-relaxed text-slate-500 dark:text-slate-400 mt-1 max-w-xl">
          {t("runtime.components.reviewer.reviewer-dashboard.text_track_your_review_workload_and_quickly")}{" "}</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-sm font-bold tracking-wide text-[#1B3C53] dark:text-white uppercase">
            {t("runtime.components.reviewer.reviewer-dashboard.text_recent_assignments")}{" "}</h2>
        </div>
        {recentAssignments.length === 0 ? (
          <div className="px-4 py-6 text-sm text-slate-500 dark:text-slate-400">
            {t("runtime.components.reviewer.reviewer-dashboard.text_no_recent_assignments")}{" "}</div>
        ) : (
              ...
              <div key={assignment.assignment_id} className="px-4 py-3 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    {assignment.conference_name} {t("runtime.components.reviewer.reviewer-dashboard.text_status")}{" "}{assignment.status}
                  </div>
                </div>
                <Link
                ...
                >
                  {t("runtime.components.reviewer.reviewer-dashboard.text_open")}{" "}</Link>
              </div>
            ))}
          </div>
        )}
