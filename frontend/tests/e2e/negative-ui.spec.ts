...
test.describe('Negative UI Tests - Frontend Validation & RBAC', () => {
  ...
  test('[UI-SEC-01] Author and Reviewer cannot access Chair-only features', async ({ page, request }) => {
    ...
    const coiDashboardLink = page.locator('a:has-text("COI Dashboard"), a:has-text("COI"), button:has-text("COI Dashboard")');
  ...
  test('[UI-NEG-02] Submit button disabled after deadline', async ({ page, request }) => {
    ...
    conferenceData.configurations.abstract_submission_deadline = new Date(pastDate.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString();
    ...
    const deadlineWarning = page.locator('text=/deadline.*passed|submission.*closed|past.*deadline|deadline.*expired/i');
  ...
  test('[UI-NEG-03] Frontend validation for invalid review scores', async ({ page, request }) => {
    ...
    if (await scoreInput.count() > 0) {
      ...
      if (await submitButton.count() > 0) {
        const isDisabled = await submitButton.isDisabled();
        console.log(`✓ Submit button state with valid score: ${isDisabled ? 'disabled (form incomplete)' : 'enabled'}`);
      }
    }
