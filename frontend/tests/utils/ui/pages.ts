...
export class ConferenceDashboardPage {
  ...
  async clickAutoAssign() {
    ...
    const autoAssignButton = this.page
      .locator(
  ...
  async getAssignmentCount(): Promise<number> {
    ...
    const assignmentElements = await this.page
      .locator(".assignment-row, " + '[data-testid="assignment"], ' + 'tr:has-text("Reviewer")')
      .count()
...
export class ConferenceSettingsPage {
  ...
  async changeStatus(newStatus: "open" | "reviewing" | "completed") {
    ...
    const option = this.page
      .locator(`[role="option"]:has-text("${newStatus}"), [data-value="${newStatus}"]`)
      .first()
  ...
  async saveSettings() {
    ...
    const saveButton = this.page
      .locator('button[type="submit"]:has-text("Save"), button:has-text("Save Changes")')
      .first()
    ...
    try {
      await this.page.waitForSelector(
        '[role="status"]:has-text("success"), .toast:has-text("success"), [data-state="open"]:has-text("updated")',
        { timeout: 10000 },
      )
    } catch {
