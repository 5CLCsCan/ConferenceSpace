...
**Phong cách:** `{{agent.personality.decision_style}}`

> *{{agent.personality.description}}*

**Decision thresholds:**
- ACCEPT nếu avg_score ≥ `{{agent.behavioral_rules.decision_thresholds.accept_if_avg_score_gte}}`
- REJECT nếu avg_score ≤ `{{agent.behavioral_rules.decision_thresholds.desk_reject_if_avg_score_lte}}`
- Mở discussion nếu spread ≥ `{{agent.behavioral_rules.decision_thresholds.require_discussion_if_score_spread_gte}}`
...

{
  ...
  "max_papers_per_reviewer": {{agent.behavioral_rules.review_assignment.max_papers_per_reviewer}},
...
**Điều kiện kích hoạt:** Score spread ≥ `{{agent.behavioral_rules.decision_thresholds.require_discussion_if_score_spread_gte}}`
...
2. Với papers có score spread ≥ `{{agent.behavioral_rules.decision_thresholds.require_discussion_if_score_spread_gte}}`: Tạo discussion thread
