...
from app.workflows.reviewer_initial_analysis.document import (
    MAX_MANUSCRIPT_ABSTRACT_CHARS,
...
from app.workflows.reviewer_initial_analysis.schemas import (
    ...
    ReviewerInitialAnalysisCacheMetadata,
    ReviewerInitialAnalysisError,
    ReviewerInitialAnalysisResolveRequest,
    ReviewerInitialAnalysisResolveResponse,
    ReviewerInitialReadinessSignal,
)
...
class ReviewerInitialAnalysisRunner:
    ...
    async def resolve(
        ...
        *,
        ...
        if cached is not None:
            return ReviewerInitialAnalysisResolveResponse.model_validate(cached)
        ...
        if request.action == "lookup":
            if latest is None:
                return ReviewerInitialAnalysisResolveResponse(
                    status="idle",
            ...
            return ReviewerInitialAnalysisResolveResponse.model_validate(
                {
                    "status": "stale",
        ...
        if extracted_document.text_coverage_ratio <= MIN_TEXT_COVERAGE_RATIO or not extracted_document.raw_text.strip():
            return await self._save_failed(
                ...
                message="uploaded manuscript does not contain enough extractable text for reviewer initial analysis",
            )
        ...
        return ReviewerInitialAnalysisResolveResponse.model_validate(response_payload)
    ...
    async def _generate_artifact(
        ...
        *,
        ...
        payload = await self._llm_client.complete_structured(
            messages=[
                {"role": "system", "content": REVIEWER_INITIAL_ANALYSIS_SYSTEM_PROMPT},
                {
                    "role": "user",
                    "content": json.dumps(
                        inference_payload,
    ...
    async def _save_failed(
        ...
        *,
        ...
        run_id = str(uuid4())
        response = ReviewerInitialAnalysisResolveResponse(
            status="failed",
            run_id=run_id,
            cache=ReviewerInitialAnalysisCacheMetadata(
                hit=False,
            ...
            error=ReviewerInitialAnalysisError(code=code, message=message),
        )
...
) -> list[ReviewerInitialReadinessSignal]:
...
def _signal_item(*, label: str, status: str, detail: str) -> ReviewerInitialReadinessSignal:
    return ReviewerInitialReadinessSignal(
        label=label,
        status=status,
        detail=detail,
        source="derived",
