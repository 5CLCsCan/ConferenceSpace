...
from app.db.models import ReviewerBriefingArtifact, ReviewerBriefingRun, ReviewerBriefingStageRecord


class ReviewerBriefingRepository:
    ...
    async def get_matching_artifact(
        ...
        *,
        ...
        async with self._session_factory() as db:
            row = (
                await db.execute(
                    select(ReviewerBriefingArtifact).where(
                        ReviewerBriefingArtifact.conference_id == conference_id,
                        ReviewerBriefingArtifact.assignment_id == assignment_id,
                        ReviewerBriefingArtifact.submission_id == submission_id,
                        ReviewerBriefingArtifact.actor_id == actor_id,
                        ReviewerBriefingArtifact.submission_state_fingerprint == submission_state_fingerprint,
                    )
                )
            ).scalar_one_or_none()
            if row is None:
    ...
    async def get_latest_artifact_for_scope(
        ...
        *,
        ...
        async with self._session_factory() as db:
            row = (
                await db.execute(
                    select(ReviewerBriefingArtifact)
                    .where(
                        ReviewerBriefingArtifact.conference_id == conference_id,
                        ReviewerBriefingArtifact.assignment_id == assignment_id,
                        ReviewerBriefingArtifact.submission_id == submission_id,
                    ...
                    .order_by(desc(ReviewerBriefingArtifact.generated_at))
                    .limit(1)
                )
            ).scalar_one_or_none()
            if row is None:
