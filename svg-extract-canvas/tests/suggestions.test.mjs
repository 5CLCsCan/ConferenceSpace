...
test('apply_extract_target_suggestions inserts bound suggested boxes', async () => {
  ...
  assert.equal(inserted[0].typeName, 'shape')
  assert.equal(inserted[0].index, 'a0')
  assert.equal(inserted[0].opacity, 1)
  assert.equal(inserted[0].isLocked, false)
  assert.equal(inserted[0].props.growY, 0)
  assert.equal(inserted[0].props.url, '')
  assert.equal(inserted[0].props.scale, 1)
  assert.equal(inserted[0].props.labelColor, 'black')
  assert.equal(inserted[0].props.font, 'draw')
  assert.equal(inserted[0].props.align, 'middle')
  assert.equal(inserted[0].props.verticalAlign, 'middle')
  assert.deepEqual(inserted[0].props.richText, { type: 'doc', content: [{ type: 'paragraph' }] })
})
