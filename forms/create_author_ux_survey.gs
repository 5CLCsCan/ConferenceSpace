/**
 * ConferenceSpace — Author UX Survey
 *
 ...
 * 2. Paste this file
 * 3. Run createAuthorUxSurvey() once (authorize when prompted)
...
function createAuthorUxSurvey() {
  var formData = getAuthorSurveyData_();
...
function installSubmitTrigger() {
  ...
  for (var i = 0; i < existing.length; i++) {
    if (existing[i].getHandlerFunction() === 'onAuthorSurveySubmit') {
      ScriptApp.deleteTrigger(existing[i]);
    }
  }

  ScriptApp.newTrigger('onAuthorSurveySubmit')
...
function onAuthorSurveySubmit(e) {
  if (!e || !e.response) {
    return;
  }
...
function addFormItem_(form, item) {
  ...
  if (type === 'MULTIPLE_CHOICE') {
    ...
    mc.setChoiceValues((item.choices || []).map(function(c) {
      return c.value;
    }));
...
function getAuthorSurveyData_() {
  return {
    title: 'Khảo sát mức độ hài lòng - Vai trò Tác giả- ConferenceSpace',
    description:
      'Khảo sát này dành cho người đã thử quy trình tác giả: xem hội nghị, nộp bài, nhập thông tin bài báo, tải tệp, khai báo xung đột lợi ích và sử dụng các tính năng AI nếu có.\n\nMục tiêu là đo mức độ hài lòng, độ dễ sử dụng, độ rõ ràng, tính năng AI thông minh và các điểm cần cải thiện của hệ thống.',
    ...
    items: [
      ...
      {
        type: 'CHECKBOX',
        title: 'Bạn đã thử những phần nào trong vai trò Tác giả?',
        hasOtherOption: true,
        choices: [
          { value: 'Tìm hội nghị phù hợp để nộp bài' },
          { value: 'Xem thông tin tổng quan, lời mời nộp bài, ngày quan trọng và ban tổ chức' },
          { value: 'Nhập thông tin bài báo, tác giả, từ khóa và track' },
          { value: 'Tải tệp bản thảo hoặc kiểm tra thông tin bản thảo' },
          { value: 'Khai báo xung đột lợi ích' },
          { value: 'Xem lại thông tin trước khi gửi bài' },
          { value: 'Theo dõi trạng thái bài nộp sau khi gửi' },
          { value: 'Sử dụng hoặc hiểu tính năng AI tự động điền, gợi ý track hoặc kiểm tra trước' }
        ]
      },
      ...
      { type: 'PAGE_BREAK', title: 'Mức độ hài lòng theo vai trò Tác giả' },
      {
        type: 'GRID',
        title: 'Bạn hài lòng ở mức nào với các phần sau trong vai trò Tác giả?',
        rows: [
          'Tìm hội nghị phù hợp để nộp bài',
          'Xem thông tin tổng quan, lời mời nộp bài, ngày quan trọng và ban tổ chức',
          'Nhập thông tin bài báo, tác giả, từ khóa và track',
          'Tải tệp bản thảo hoặc kiểm tra thông tin bản thảo',
          'Khai báo xung đột lợi ích',
          'Xem lại thông tin trước khi gửi bài',
          'Theo dõi trạng thái bài nộp sau khi gửi',
          'Sử dụng hoặc hiểu tính năng AI tự động điền, gợi ý track hoặc kiểm tra trước'
        ],
        columns: [
          'Rất không hài lòng',
          'Không hài lòng',
      ...
      },
      {
        ...
        rows: [
          'Quy trình nộp bài được chia thành các bước hợp lý.',
          'Tôi hiểu rõ thông tin nào là bắt buộc trước khi gửi bài.',
          'Thông báo lưu bản nháp, gửi bài và trạng thái bài nộp rõ ràng.',
          'Các câu hỏi về xung đột lợi ích dễ hiểu và không gây cảm giác xâm phạm.',
          'Tính năng AI dành cho tác giả giúp tôi giảm thao tác thủ công.',
          'Tính năng AI giúp tôi phát hiện hoặc tránh lỗi trước khi gửi bài.',
          'Tôi cảm thấy tự tin rằng bài nộp đã được gửi đúng cách.'
        ],
        columns: [
      ...
      { type: 'PAGE_BREAK', title: 'Độ rõ ràng và khả năng sử dụng' },
      {
        ...
        rows: [
          'Tôi hiểu mục đích của từng màn hình chính trong vai trò Tác giả.',
      ...
      { type: 'PAGE_BREAK', title: 'Ưu tiên cải thiện' },
      {
        ...
        hasOtherOption: true,
        choices: [
          { value: 'Xem danh sách hội nghị' },
          { value: 'Trang chi tiết hội nghị' },
          { value: 'Biểu mẫu nộp bài' },
          { value: 'Lưu bản nháp' },
          { value: 'Tải tệp bản thảo' },
          { value: 'Khai báo xung đột lợi ích' },
          { value: 'AI tự động điền' },
          { value: 'AI gợi ý track' },
          { value: 'AI kiểm tra trước hoặc tín hiệu chất lượng' },
          { value: 'Theo dõi trạng thái bài nộp' },
