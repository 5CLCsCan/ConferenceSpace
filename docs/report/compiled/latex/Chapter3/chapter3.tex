...
Chương 2 đã chuyển nhu cầu của người dùng thành các yêu cầu hệ thống và nguyên tắc thiết kế cho ConferenceSpace. Chương này trình bày cách nhóm xây dựng hệ thống theo các yêu cầu đó, từ use case, kiến trúc và dữ liệu đến cơ chế nghiệp vụ, thuật toán xác định, các chức năng AI và môi trường vận hành. Mỗi phần làm rõ tác vụ được xử lý, kết quả được tạo ra và chủ thể chịu trách nhiệm trước khi hệ thống cập nhật dữ liệu nghiệp vụ.
...
Hình~\ref{fig:ch3-responsibility-model-cut} cho thấy đề xuất của thuật toán và đầu ra AI không tự cập nhật trạng thái hệ thống. Người dùng có thẩm quyền phải lựa chọn hành động tiếp theo; backend chỉ ghi dữ liệu sau khi kiểm tra quyền và điều kiện nghiệp vụ. Ranh giới này duy trì trách nhiệm học thuật ngay cả khi hệ thống cung cấp mức độ hỗ trợ khác nhau cho từng vai trò.
...
AI hỗ trợ từng vai trò tại các điểm nghẽn cụ thể nhưng không thay đổi thẩm quyền học thuật. Tác giả xác nhận dữ liệu bài nộp; Phản biện viên tự đọc bài và chịu trách nhiệm về nội dung phản biện; Chủ tọa xác nhận phân công và quyết định chấp nhận hoặc từ chối. Quản trị viên, dịch vụ AI và tác vụ nền là các tác nhân hỗ trợ vận hành, không phải vai trò nghiệp vụ chính trong phạm vi báo cáo.
...
\begin{figure}[H]
  \centering
  \begin{adjustbox}{max width=\textwidth, max height=0.86\textheight, keepaspectratio}
    \begin{tikzpicture}[
        uc/.style={draw=black!65, ellipse, align=center, font=\footnotesize, text width=3.7cm, minimum height=11mm, fill=blue!6},
        shared/.style={draw=black!65, ellipse, align=center, font=\footnotesize, text width=3.7cm, minimum height=11mm, fill=orange!9},
        actorlabel/.style={font=\footnotesize\bfseries, align=center, text width=2.7cm},
        assoc/.style={-, thick, draw=black!65},
        actorfig/.pic={
          \draw[thick, fill=gray!15] (0,0.62) circle (0.16);
          \draw[thick] (0,0.46) -- (0,-0.05);
          \draw[thick] (-0.28,0.28) -- (0.28,0.28);
          \draw[thick] (0,-0.05) -- (-0.22,-0.5);
          \draw[thick] (0,-0.05) -- (0.22,-0.5);
        }
      ]
      \draw[rounded corners=3pt] (-7.7,4.0) rectangle (7.7,-9.6);
      \node[font=\small\bfseries] at (0,3.55) {ConferenceSpace};

      % Actor: hình que + nhãn bên dưới; coordinate đặt tại thân hình để nối association
      \path (-10.0,1.85) pic {actorfig};
      \coordinate (author) at (-10.0,1.85);
      \node[actorlabel] at (-10.0,0.95) {Tác giả};

      \path (10.0,1.85) pic {actorfig};
      \coordinate (reviewer) at (10.0,1.85);
      \node[actorlabel] at (10.0,0.95) {Phản biện viên};

      \path (0,-10.7) pic {actorfig};
      \coordinate (chair) at (0,-10.7);
      \node[actorlabel] at (0,-11.6) {Chủ tọa /\\Đồng chủ tọa};

      \node[uc] (uc01) at (-4.5,2.0) {UC-01\\Khám phá hội nghị};
      \node[uc] (uc02) at (-4.5,0.0) {UC-02\\Hoàn tất bài nộp};
      \node[uc] (uc03) at (-4.5,-2.0) {UC-03\\Vòng đời bài nộp};
      \node[uc] (uc04) at (4.5,2.0) {UC-04\\Lời mời hội nghị và bài được phân công};
      \node[uc] (uc05) at (4.5,0.0) {UC-05\\Soạn và gửi phản biện};
      \node[shared] (uc08) at (0,-3.5) {UC-08\\Trao đổi theo bài nộp};
      \node[shared] (uc10) at (0,-5.5) {UC-10\\Chatbot Agent};
      \node[uc] (uc06) at (-4.5,-7.8) {UC-06\\Phân công và COI};
      \node[uc] (uc07) at (0,-7.8) {UC-07\\Quản lý hội nghị};
      \node[uc] (uc09) at (4.5,-7.8) {UC-09\\Hỗ trợ ra quyết định};

      \foreach \n in {uc01,uc02,uc03} \draw[assoc] (author) -- (\n);
      \foreach \n in {uc04,uc05} \draw[assoc] (reviewer) -- (\n);
      \foreach \n in {uc06,uc07,uc09} \draw[assoc] (chair) -- (\n);

      \draw[assoc] (author) -- (uc08);
      \draw[assoc] (author) -- (-6.6,-4.3) -- (-1.0,-4.3) -- (uc10);
      \draw[assoc] (reviewer) -- (uc08);
      \draw[assoc] (reviewer) -- (uc10);
      \draw[assoc] (chair) -- (0,-8.7) -- (2.3,-8.7) -- (2.3,-3.5) -- (uc08);
      \draw[assoc] (chair) -- (0,-8.7) -- (2.0,-8.7) -- (2.0,-5.5) -- (uc10);
    \end{tikzpicture}
  \end{adjustbox}
  \caption{Các use case tổng quát theo vai trò trong ConferenceSpace}
...
Sơ đồ cho thấy Tác giả có thể rút bài đã gửi khi cấu hình và hạn chót cho phép. Sau khi phản biện được công bố, Chủ tọa hoặc Đồng chủ tọa mở và kết thúc giai đoạn phản hồi; Tác giả gửi phản hồi, còn Phản biện viên có thể cập nhật điểm, khuyến nghị và nhận xét sau khi đọc phản hồi. Các thay đổi này cập nhật gói bằng chứng trước khi Chủ tọa ra quyết định. Chỉ bài ở trạng thái \textit{accepted} mới chuyển sang bước nộp bản thảo hoàn chỉnh; backend kiểm tra điều kiện trước khi ghi trạng thái mới.
...
\begin{longtable}{|P{0.18\textwidth}|P{0.20\textwidth}|P{0.17\textwidth}|P{0.20\textwidth}|P{0.17\textwidth}|}
  ...
  Điều phối & Bài đã gửi; hội đồng đã mời & Chủ tọa, Phản biện viên & Chấp nhận lời mời hội nghị; tạo và xem phân công & Phân công chờ xử lý hoặc đang thực hiện \\ \hline
  Phản biện & Phân công hợp lệ & Phản biện viên & Đọc bài, lưu nháp và gửi phản biện & Phản biện đã gửi \\ \hline
  Phản hồi và thảo luận & Phản biện được công bố & Chủ tọa/Đồng chủ tọa, Tác giả, Phản biện viên & Mở hoặc kết thúc giai đoạn; gửi phản hồi; cập nhật đánh giá và trao đổi theo quyền & Gói bằng chứng được cập nhật \\ \hline
...
Tác giả tìm kiếm hội nghị theo từ khóa, lĩnh vực, chuyên đề hoặc trạng thái, sau đó xem lời mời nộp bài và các hạn chót liên quan. Hệ thống chỉ cho phép bắt đầu bài nộp khi hội nghị còn mở; hội nghị đã đóng vẫn có thể được xem hoặc lưu theo dõi. Use case kết thúc khi Tác giả chọn hội nghị để nộp bài hoặc lưu hội nghị vào danh sách quan tâm.
...
Hình~\ref{fig:uc01-detail-committee-cut} thể hiện tab hội đồng ban chương trình trên trang chi tiết hội nghị.
...
Tác giả tạo bản nháp, tải bản thảo và có thể yêu cầu Submission Autofill trích xuất siêu dữ liệu cùng gợi ý chuyên đề trong danh sách hợp lệ. Tác giả kiểm tra, chỉnh sửa thông tin, khai báo đồng tác giả và xung đột lợi ích trước khi Submission Gating kiểm tra tệp, chính sách và các điều kiện gửi bài. Trạng thái \textit{warn} cho phép tiếp tục khi chính sách hội nghị cho phép; trạng thái \textit{block} ngăn gửi cho đến khi lỗi có căn cứ được khắc phục. Nếu Submission Autofill không khả dụng, Tác giả vẫn có thể nhập và lưu bản nháp thủ công. Nếu toàn bộ Submission Gating không trả kết quả hợp lệ, backend chưa cho phép công bố bài cho đến khi thực hiện được một lần kiểm tra hợp lệ.
...
Trước khi gửi, Hình~\ref{fig:uc02-gating-cut} thể hiện kết quả kiểm tra tự động của Submission Gating theo ba trạng thái \textit{pass}, \textit{warn} và \textit{block}.
...
Sau khi tạo bài, Tác giả theo dõi và thực hiện các hành động phù hợp với từng trạng thái: chỉnh sửa bản nháp; theo dõi hoặc rút bài đã gửi khi cấu hình và hạn chót cho phép; xem phản biện khi được công bố; gửi phản hồi trong giai đoạn do Chủ tọa hoặc Đồng chủ tọa mở; và tải bản thảo hoàn chỉnh khi bài được chấp nhận. Backend từ chối yêu cầu không phù hợp với trạng thái hoặc hạn chót. Phiên bản hiện tại hỗ trợ tải và truy xuất bản thảo hoàn chỉnh nhưng chưa có luồng để Chủ tọa phê duyệt, yêu cầu nộp lại hoặc kiểm tra hạn chót của bước này tại thời điểm thực thi.
...
Chủ tọa gửi lời mời tham gia hội nghị; hệ thống lưu trạng thái và thông báo cho người nhận. Phản biện viên chỉ có thể chấp nhận hoặc từ chối lời mời của chính mình. Đây là vòng đời vai trò trong hội nghị, khác với trạng thái của một phân công bài cụ thể. Khi Chủ tọa đã tạo phân công, Phản biện viên xem bài, hạn chót và trạng thái trên bảng điều khiển; backend chỉ trả các phân công thuộc người dùng hiện tại. Kho lưu trữ có thao tác phản hồi phân công theo định danh, nhưng luồng giao diện được mô tả trong chương này không đồng nhất thao tác đó với việc chấp nhận lời mời hội nghị.
...
Sau khi phân công được tạo, Hình~\ref{fig:uc04-accept-deny-cut} cho thấy giao diện chi tiết bài viết để Phản biện viên lựa chọn đồng ý hoặc từ chối.
...
% \begin{figure}[H]
%   \centering
%   \makebox[\textwidth][c]{\includegraphics[width=1.10\textwidth]{images/chapter_3_uc05_reviewer_initial_analysis.png}}
...
% \begin{figure}[H]
%   \centering
%   \makebox[\textwidth][c]{\includegraphics[width=1.10\textwidth]{images/chapter_3_uc05_reviewer_quality_auditor.png}}
%   \caption{Bảng rà soát bản nháp của Review Quality Auditor}
%   \label{fig:uc05-rqa-cut}
% \end{figure}

Trước khi gửi, Hình~\ref{fig:uc05-rqa-cut} trình bày bảng rà soát bản nháp do Review Quality Auditor tạo ra.

Review Quality Auditor chạy trước khi gửi để đưa các phát hiện vào bảng rà soát. Phát hiện mức \textit{blocking} được làm nổi bật nhưng không ngăn Phản biện viên gửi phản biện. Nếu tác vụ rà soát không thể hoàn tất do lỗi dịch vụ, backend yêu cầu Phản biện viên xác nhận tiếp tục mà không có kết quả rà soát và ghi sự kiện này vào nhật ký. Hai trường hợp được tách biệt để cảnh báo chất lượng không thay thế quyền quyết định của Phản biện viên, trong khi lỗi thực thi vẫn được công khai và truy vết.

\subsubsection{UC-06. Phân công phản biện có kiểm tra xung đột lợi ích}

Hệ thống chuẩn bị các cặp bài nộp--Phản biện viên, tính điểm phù hợp, loại các cặp có xung đột lợi ích khỏi đề xuất tự động và theo dõi số phân công được tạo trong cùng lượt chạy. Chủ tọa xem các đề xuất đã được tạo cùng điểm số, lý do đối sánh, trạng thái kiểm tra COI, tải phát sinh trong lượt chạy và danh sách bài chưa đủ Phản biện viên trước khi điều chỉnh hoặc xác nhận. Hợp đồng \CodeBreak{AutoAssignResponse} không trả danh sách đầy đủ từng cặp ứng viên bị loại. Với thao tác thêm đề xuất thủ công, backend kiểm tra quyền và trả cảnh báo xung đột lợi ích nhưng hiện chưa ngăn lưu; vì vậy, Chủ tọa phải xử lý cảnh báo trước khi xác nhận phương án.
...
Công thức tính điểm, thủ tục phân công, lượt dự phòng và các trường hợp ngoại lệ được trình bày tại Mục~3.4. Thuật toán tự động không nới ràng buộc xung đột lợi ích, kể cả khi hệ thống không tìm được đủ Phản biện viên; luồng thêm đề xuất thủ công hiện dùng cảnh báo thay cho cưỡng chế tại thời điểm ghi dữ liệu.

\subsubsection{UC-07. Quản lý hội nghị và theo dõi tiến độ}

Chủ tọa cấu hình thông tin hội nghị, chuyên đề, hạn chót, biểu mẫu phản biện và chính sách; mời thành viên hội đồng chương trình; theo dõi số bài, tiến độ phản biện và các trường hợp cần xử lý. Hệ thống từ chối cấu hình thiếu trường bắt buộc, hạn chót không hợp lệ hoặc thao tác của người dùng không có quyền. Bảng điều khiển hỗ trợ nhận biết điểm nghẽn nhưng không tự chuyển giai đoạn hoặc ra quyết định thay Chủ tọa.
...
Hình~\ref{fig:uc07-config-cut} thể hiện giao diện thiết lập cấu hình thông tin và hạn chót hội nghị.
...
Hình~\ref{fig:uc07-dashboard-cut} minh họa các thông tin điều phối chính được cung cấp cho Chủ tọa, qua đó cân bằng hai ảnh hỗ trợ chuyên biệt về đối sánh và tổng hợp quyết định.
...
Chủ tọa hoặc Đồng chủ tọa mở giai đoạn phản hồi và kết thúc giai đoạn này trước khi ra quyết định; giai đoạn thảo luận chỉ được mở khi cấu hình hội nghị cho phép. Phản biện viên được phân công có thể tạo chuỗi thảo luận; Tác giả của bài và Phản biện viên trao đổi trong chuỗi tương ứng, còn Chủ tọa xem toàn bộ lịch sử ở chế độ giám sát. Sau khi đọc phản hồi, Phản biện viên có thể cập nhật điểm, khuyến nghị và nhận xét sau phản hồi trên phân công của mình. Backend kiểm tra quan hệ giữa người dùng, bài nộp và chuỗi thảo luận trước mỗi thao tác. Nội dung trao đổi cùng đánh giá cập nhật được lưu với bài nộp và trở thành nguồn bằng chứng cho Chair Decision Copilot.
...
Chatbot Agent nhận câu hỏi, lịch sử hội thoại và ngữ cảnh trang hiện tại. Nếu cần dữ liệu hệ thống, công cụ gửi yêu cầu có cấu trúc đến backend; backend kiểm tra mã xác thực, tài nguyên, trường dữ liệu, bộ lọc và quyền của người dùng trước khi trả phần dữ liệu tối thiểu được phép. Chatbot Agent dùng kết quả này để tạo câu trả lời phù hợp với vai trò và không truy cập trực tiếp cơ sở dữ liệu.

Yêu cầu vượt phạm vi chức năng hoặc quyền truy cập bị từ chối rõ ràng. Khi công cụ gặp lỗi, hệ thống trả trạng thái lỗi thay vì tạo dữ liệu không có căn cứ. Các kịch bản hội thoại ở Chương 4 kiểm tra ranh giới này trong phạm vi tài nguyên và vai trò đã chọn.
...
Để minh họa trực quan cho luồng tương tác này, giao diện hội thoại của Chatbot Agent được thể hiện tại Hình~\ref{fig:uc10-chatbot-cut}.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}[node distance=12mm and 18mm, >=Latex]
      ...
      \draw[prismDashedArrow, bend left=24] (ai) to node[prismEdgeLabel] {backend query endpoint} (backend);
...
Backend là ranh giới nghiệp vụ chính đối với dữ liệu và cập nhật trạng thái. Giao diện không truy cập trực tiếp cơ sở dữ liệu hoặc nhà cung cấp mô hình; tuy nhiên, Next.js có luồng giao tiếp trực tiếp với dịch vụ AI cho hội thoại. Khi Chatbot Agent cần dữ liệu nghiệp vụ, dịch vụ AI gọi backend query endpoint bằng mã xác thực của người dùng và mã xác thực liên dịch vụ; backend tiếp tục kiểm tra danh mục tài nguyên và RBAC. Sơ đồ vì vậy tách luồng hội thoại Next.js--AI khỏi luồng truy vấn dữ liệu AI--backend.
...
Giao diện sử dụng Next.js App Router, React, TypeScript, Radix UI và Tailwind CSS \cite{ch3ref4,ch3ref5}. Các luồng dài được chia thành từng bước; trạng thái quan trọng được thể hiện bằng nhãn và bảng; thao tác gửi bài, gửi phản biện và lưu quyết định yêu cầu người dùng xác nhận. Đầu ra AI được trình bày dưới dạng bản nháp, cảnh báo hoặc bản tổng hợp cần kiểm tra, thay vì kết luận chính thức.

\subsection{Thiết kế backend, API và phân quyền}
\label{subsec:ch3-backend-cut}

Backend áp dụng kiến trúc phân lớp. Tầng điều khiển tiếp nhận yêu cầu HTTP; tầng dịch vụ thực thi nghiệp vụ; tầng lưu trữ làm việc với PostgreSQL; tầng client kết nối dịch vụ AI, Neo4j, nguồn dữ liệu học thuật và thư điện tử. Nghiệp vụ phân công được tách thành miền riêng để logic tính điểm, đối sánh và phát hiện xung đột lợi ích có thể được kiểm thử độc lập.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=0.9\textwidth}
    \begin{tikzpicture}[node distance=12mm and 18mm, >=Latex]
      ...
      \node[prismNode] (coi) at (0,-11.4) {Đối sánh và COI};
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|P{0.22\textwidth}|Y|Y|Y|}
    ...
    Phản biện viên được phân công & Có & Các chuỗi do mình tạo trong bài nộp & Có, trong chuỗi do mình sở hữu \\ \hline
...
Ngoài JWT dành cho người dùng cuối, hệ thống dùng mã xác thực riêng cho tác vụ quản trị nội bộ và giao tiếp giữa dịch vụ AI với backend query endpoint. Mã xác thực dịch vụ không thay thế kiểm tra quyền: backend vẫn giới hạn tài nguyên, trường dữ liệu và thao tác theo người dùng hiện tại.
...
Hình~\ref{fig:ch3-data-cut} giữ nguyên định danh kỹ thuật và phân biệt khóa ngoại vật lý với quan hệ được nối ở tầng ứng dụng. \CodeBreak{paper\_assignments.reviewer\_id} tham chiếu \CodeBreak{conference\_reviewers.id}; từ đó hệ thống mới xác định người dùng giữ vai trò Phản biện viên trong một hội nghị. Nét đứt không biểu thị ràng buộc khóa ngoại trong PostgreSQL. \CodeBreak{coi\_relationships} lưu bằng chứng do quy trình refresh hoặc rebuild tạo ra, còn conflict map dùng để loại cặp xung đột được xây dựng riêng cho từng lượt đối sánh.

Các luồng AI lưu dữ liệu truy vết theo hợp đồng riêng. Trong chương này, fingerprint là dấu định danh được tính từ trạng thái dữ liệu đầu vào; \textit{stale} chỉ kết quả không còn hiệu lực vì dữ liệu nguồn đã thay đổi. Reviewer Initial Analysis dùng fingerprint của trạng thái bài nộp và có trạng thái \textit{stale}; Chair Decision Copilot dùng fingerprint của gói bằng chứng, fingerprint của từng thành phần và lý do hết hiệu lực. Submission Gating ghi fingerprint đầu vào cùng mã băm chính sách cho mỗi lần chạy. Review Quality Auditor lưu yêu cầu, phản hồi, trạng thái lần chạy và fingerprint của điều kiện phát hiện nhưng chưa đánh dấu lần rà soát cũ là \textit{stale}. Submission Autofill trả kết quả để Tác giả áp dụng vào biểu mẫu có thể chỉnh sửa, còn Chatbot Agent lưu phiên và lịch sử hội thoại; hai luồng này chưa lưu kết quả dưới dạng bản ghi có trạng thái \textit{stale}.

\subsection{Luồng xử lý hệ thống}

\subsubsection{Luồng nộp bài có AI hỗ trợ}

Trong luồng nộp bài, giao diện chuyển tệp và ngữ cảnh hội nghị đến backend; backend gọi Submission Autofill để tạo bản nháp, sau đó lưu các chỉnh sửa do Tác giả xác nhận. Trước khi gửi chính thức, backend gọi Submission Gating trên trạng thái bản nháp hiện hành. AI chỉ tạo dữ liệu hỗ trợ và cảnh báo; backend kiểm soát việc ghi dữ liệu chính thức, còn Tác giả quyết định gửi bài. Tác giả có thể tiếp tục nhập và lưu bản nháp khi chức năng AI không khả dụng; thao tác công bố vẫn cần một kết quả Submission Gating hợp lệ.

\subsubsection{Luồng Chatbot Agent truy vấn dữ liệu hệ thống}

Chatbot Agent chỉ gọi backend query endpoint đã công bố. Mỗi yêu cầu gồm ngữ cảnh người dùng, tài nguyên, trường dữ liệu và điều kiện truy vấn; backend xác thực người dùng và dịch vụ, kiểm tra danh mục tài nguyên rồi mới thực thi. Cách tổ chức này giữ việc kiểm soát quyền tại backend và ngăn Chatbot Agent sử dụng quyền truy cập cơ sở dữ liệu trực tiếp.

\section{Cơ chế nghiệp vụ và thuật toán xác định}

\subsection{Vai trò của các cơ chế xác định}

Các tác vụ có quy tắc rõ ràng được tách khỏi lớp AI để người dùng có thể kiểm tra điều kiện và nguyên nhân của kết quả. Nhóm này gồm đối sánh Phản biện viên, phát hiện xung đột lợi ích, kiểm soát trạng thái và hạn chót, phân quyền, định tuyến thông báo và kiểm tra cấu trúc dữ liệu. Mức ổn định của từng cơ chế phụ thuộc vào việc đã định nghĩa đầy đủ quy tắc, bao gồm cách xử lý trường hợp bằng điểm.

\subsection{Đối sánh phản biện viên}
\label{subsec:ch3-matching-cut}

Hệ thống tạo đề xuất phân công từ miền chuyên môn của bài nộp và Phản biện viên, cấu hình hội nghị cùng kết quả kiểm tra xung đột lợi ích. Matcher hiện chỉ nhận định danh Phản biện viên; bộ đếm tải được khởi tạo bằng $0$ và chỉ phản ánh số phân công phát sinh trong lượt chạy, không bao gồm các phân công đã tồn tại trước đó. Với tập miền chuyên môn của bài nộp $S$ và của Phản biện viên $R$, điểm phù hợp được tính bằng hệ số Jaccard:

\[
  J(S,R)=\frac{|S\cap R|}{|S\cup R|}.
\]

Nếu cả hai tập rỗng, hệ thống gán điểm $0$ vì không có bằng chứng về mức độ phù hợp. Nhóm chọn Jaccard vì điểm số có thể giải thích trực tiếp từ số miền chuyên môn chung và không cần mô hình học máy hoặc dữ liệu huấn luyện. Chiến lược tham lam lần lượt chọn các cặp có điểm cao, nhờ đó đơn giản hóa việc triển khai và kiểm tra so với cách giải bài toán tối ưu toàn cục. Đổi lại, Jaccard xem các miền chuyên môn như tập hợp không trọng số; chiến lược tham lam có thể bỏ lỡ phương án có tổng mức phù hợp cao hơn cho toàn bộ hội nghị và phụ thuộc vào thứ tự khi nhiều cặp bằng điểm.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=0.90\textwidth, max height=0.70\textheight, keepaspectratio}
    \begin{tikzpicture}[scale=0.84, every node/.style={transform shape},
      ...
      \node[pass] (pass1) at (0,-7.3) {\textbf{Pass 1}\newline Giữ ngưỡng điểm, số người cần cho mỗi bài, giới hạn tải phát sinh trong lượt chạy và COI};
      \node[decision] (unassigned) at (0,-10.7) {Bài nào vẫn có 0 Phản biện viên?};
      \node[pass] (pass2) at (5.8,-13.8) {\textbf{Pass 2}\newline Bỏ ngưỡng điểm và giới hạn tải; vẫn giữ COI; chỉ bổ sung một Phản biện viên};
...
Thuật toán giảm công sức tìm kiếm và cung cấp căn cứ để Chủ tọa kiểm tra trước khi xác nhận. Bộ đếm tải chỉ bao gồm các phân công phát sinh trong lượt chạy hiện tại, còn trường hợp bằng điểm chưa có tiêu chí phân xử thứ cấp; đây là hai ranh giới trực tiếp của phương án hiện hành.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}[
      ...
      \node[process] (map) at (-5.2,-3.1) {Tạo conflict map cho lượt chạy};
      \node[process] (auto) at (-5.2,-5.8) {Auto-matching loại cặp COI trong cả hai pass};

      \node[process] (rebuild) at (0,-3.1) {Tác vụ refresh/rebuild quan hệ COI};
      \node[evidence] (stored) at (0,-5.8) {Ghi bằng chứng vào \CodeBreak{coi\_relationships}};

      \node[warning] (manualCheck) at (5.2,-3.1) {Đề xuất thủ công kiểm tra trực tiếp và đọc cache COI nếu có};
      \node[warning] (manualResult) at (5.2,-5.8) {Trả \CodeBreak{COIWarning}; lưu trạng thái kiểm tra trong suggestion metadata};
      ...
      \draw[prismDashedArrow] (stored) to[bend right=16] node[prismEdgeLabel] {cache nếu có} (manualCheck);
...
Hình~\ref{fig:ch3-coi-flow-cut} tách ba mục đích sử dụng kết quả COI. Conflict map chỉ phục vụ lượt auto-matching và loại cặp xung đột trước khi xếp hạng. \CodeBreak{coi\_relationships} được cập nhật qua tác vụ refresh hoặc rebuild riêng. Khi Chủ tọa thêm đề xuất thủ công, backend kiểm tra trực tiếp quan hệ tác giả, đồng tác giả và khai báo, đọc thêm cache COI nếu có, trả \CodeBreak{COIWarning} và lưu trạng thái kiểm tra trong metadata của đề xuất; thao tác này không tự ghi quan hệ mới vào \CodeBreak{coi\_relationships}.

Thuật toán tự động không nới điều kiện COI trong lượt dự phòng. Nếu detector quan hệ không được cấu hình, hệ thống ghi \CodeBreak{skipped\_neo4j\_unavailable} và tiếp tục với các nguồn còn lại. Khi detector đã được cấu hình nhưng truy vấn Neo4j gặp lỗi, một số đường xử lý chỉ ghi nhật ký rồi tiếp tục; do đó, kết quả tổng hợp không luôn xác nhận rằng quan hệ đồng tác giả nhiều bậc đã được kiểm tra đầy đủ. Luồng thêm đề xuất thủ công hiện cảnh báo nhưng chưa chặn ghi dữ liệu và chưa tạo bản ghi kiểm toán riêng cho quyết định tiếp tục khi có COI.

\subsection{Các cơ chế nghiệp vụ hỗ trợ vận hành}
\label{subsec:ch3-operations-cut}

Các cơ chế hỗ trợ gồm máy trạng thái của hội nghị, kiểm soát hạn chót, phân quyền theo vai trò trong hội nghị (RBAC), định tuyến thông báo và kiểm tra cấu trúc dữ liệu. Chúng bảo đảm người dùng chỉ thực hiện thao tác phù hợp với vai trò và trạng thái hiện hành trước khi hệ thống ghi dữ liệu.

Cơ chế kiểm soát bản thảo hoàn chỉnh hiện chỉ kiểm tra bài ở trạng thái \textit{accepted} và tính hợp lệ của tệp; hệ thống chưa kiểm tra hạn chót của bước này tại thời điểm thực thi. Giới hạn được nêu riêng vì đầu ra AI không thể bù cho một quy tắc nghiệp vụ chưa được áp dụng đầy đủ.

\section{Giải pháp AI}

\subsection{Vai trò của AI trong hệ thống}

AI được tích hợp tại sáu điểm nghẽn cụ thể. Submission Autofill và Submission Gating hỗ trợ Tác giả hoàn tất bài nộp; Reviewer Initial Analysis và Review Quality Auditor hỗ trợ Phản biện viên đọc bài và rà soát bản nháp; Chair Decision Copilot và Chatbot Agent hỗ trợ tổng hợp hoặc truy vấn thông tin. Bảng~\ref{tab:ch3-ai-overview-cut} đối chiếu đầu ra, điểm kiểm soát và ranh giới của từng luồng.

\begingroup
\setlength{\tabcolsep}{2pt}
\begin{longtable}{|P{0.17\textwidth}|P{0.26\textwidth}|P{0.23\textwidth}|P{0.26\textwidth}|}
  \caption{Hợp đồng dữ liệu và trạng thái của các luồng AI}\label{tab:ch3-ai-overview-cut}\\
  \hline
  ...
  Submission Gating & Tệp và chính sách hội nghị & Trạng thái cùng phát hiện có bằng chứng & Ghi fingerprint đầu vào, mã băm chính sách; giữ kết quả quy tắc nếu riêng bước AI lỗi \\ \hline
  Reviewer Initial Analysis & Bản thảo và phân công & Bản định hướng đọc & Dùng fingerprint để chuyển kết quả sang \textit{stale} khi bản thảo thay đổi \\ \hline
  Review Quality Auditor & Bản nháp phản biện và điểm & Danh sách phát hiện & Lưu trạng thái lần chạy và fingerprint phát hiện; lỗi thực thi được báo riêng \\ \hline
  Chair Decision Copilot & Gói bằng chứng & Bản tổng hợp đồng thuận, bất đồng và vấn đề còn mở & Dùng fingerprint gói bằng chứng để đánh dấu \textit{stale} và nêu lý do \\ \hline
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|P{0.20\textwidth}|Y|Y|}
    ...
    Submission Gating & Tác giả sửa lỗi hoặc xem cảnh báo & Quy tắc xác định có thể \textit{block}; đánh giá AI chỉ \textit{warn}/\textit{pass} \\ \hline
    Reviewer Initial Analysis & Phản biện viên đối chiếu bài gốc & Không cập nhật điểm hoặc nội dung phản biện \\ \hline
    Review Quality Auditor & Phản biện viên xem, sửa hoặc gửi; xác nhận nếu audit thất bại & Nhãn \textit{blocking} chỉ làm nổi bật phát hiện, không ngăn gửi \\ \hline
...
Giao diện giữ toàn bộ trường ở trạng thái có thể chỉnh sửa; backend chỉ lưu bài nộp chính thức sau khi Tác giả xác nhận. Chương 4 đo độ khớp của siêu dữ liệu và kiểm tra gợi ý có thuộc danh sách chuyên đề hợp lệ. Phạm vi đánh giá chưa bao gồm nhãn chuyên gia cho thứ hạng chuyên đề; mức thời gian tiết kiệm được xác định từ phản hồi tự báo cáo, không phải phép đo trực tiếp thời gian hoàn thành biểu mẫu.

\subsubsection{Submission Gating}

Submission Gating kết hợp kiểm tra xác định và đánh giá nội dung có AI hỗ trợ. Luồng chuẩn hóa yêu cầu, kiểm tra tính toàn vẹn của tệp, trích xuất tài liệu, xác định dữ kiện, áp dụng chính sách hội nghị và tổng hợp phát hiện thành bốn trạng thái: \textit{pass}, \textit{warn}, \textit{block} hoặc \textit{error}.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}[
      ...
      \node[rule] (rules) at (-3.7,-5.4) {Kiểm tra xác định\\định dạng, số trang, phần bắt buộc, ẩn danh và các quy tắc cấu hình};
      \node[ai] (content) at (3.7,-5.4) {Đánh giá nội dung có AI hỗ trợ\\chỉ tạo \textit{warn} hoặc \textit{pass}};
      \node[stage] (merge) at (0,-8.3) {Tổng hợp phát hiện và ghi fingerprint đầu vào, mã băm chính sách};
    ...
    \end{tikzpicture}
  \end{adjustbox}
  \caption{Submission Gating phân biệt kiểm tra xác định với cảnh báo nội dung do AI hỗ trợ}
  \label{fig:ch3-gating-flow-cut}
\end{figure}

Hình~\ref{fig:ch3-gating-flow-cut} cho thấy chỉ lỗi kỹ thuật và vi phạm quy tắc xác định mới có thể chặn thao tác gửi. Khi riêng bước đánh giá nội dung lỗi, hệ thống vẫn giữ kết quả của các kiểm tra đã hoàn tất nhưng phải nêu rõ phạm vi chưa được đánh giá.

Các lỗi kỹ thuật và vi phạm quy tắc có thể tái tạo, như tệp không đọc được, thiếu phần bắt buộc, vi phạm yêu cầu ẩn danh hoặc vượt giới hạn trang, có thể tạo trạng thái \textit{block}. Đánh giá định tính của AI chỉ tạo \textit{warn} hoặc \textit{pass}; hệ thống không dùng nhận định này để từ chối bài về mặt học thuật. Nếu riêng bước đánh giá nội dung có hỗ trợ AI lỗi, luồng có thể tiếp tục tổng hợp từ các kiểm tra xác định đã hoàn tất; trạng thái \textit{pass} trong trường hợp này không có nghĩa phần nội dung đã được đánh giá đầy đủ. Nếu toàn bộ quy trình không trả kết quả hợp lệ, backend không tự động cho phép công bố bài. Mỗi phát hiện gồm nguồn, mức độ nghiêm trọng, bằng chứng và hướng dẫn khắc phục để Tác giả biết điều cần kiểm tra.

Fingerprint đầu vào và mã băm chính sách cho biết kết quả được tạo từ bản nháp và cấu hình nào. Chương 4 kiểm tra độ chính xác của tám trường hợp kiểm thử quy tắc và xác nhận 24 trường hợp cảnh báo AI không tạo \textit{block}; bộ thử chưa có nhãn người để đo độ chính xác nội dung của từng cảnh báo.

\subsubsection{Reviewer Initial Analysis}

Reviewer Initial Analysis tạo bản định hướng trước khi Phản biện viên viết nhận xét. Đầu ra gồm tóm tắt, đóng góp mà bài trình bày, điểm cần kiểm chứng và chú thích theo từng phần. Kết quả được gắn với phân công, bài nộp và fingerprint của trạng thái bài nộp; khi bản thảo thay đổi, hệ thống chuyển bản phân tích cũ sang trạng thái \textit{stale}.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|P{0.14\textwidth}|Y|Y|}
    ...
    \textit{block} & Có ít nhất một phát hiện thuộc nhóm được nền tảng gắn mức \textit{blocking} & Làm nổi bật phát hiện nghiêm trọng nhưng không ngăn gửi chính thức \\ \hline
  \end{tabularx}
\end{table}
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}[
      ...
      \node[advisory, fill=green!7] (success) at (2.0,-5.8) {Audit hoàn tất\\lưu phản biện; nhãn \textit{block} không phải điều kiện từ chối};
      \node[failure] (failure) at (7.4,-5.8) {Audit không hoàn tất\\trả \CodeBreak{review\_audit\_failed}};
...
Giao diện chạy \CodeBreak{submit\_preflight} để hiển thị các phát hiện trước khi Phản biện viên gửi. Khi nhận yêu cầu gửi chính thức, backend chạy lại \CodeBreak{submit\_enforcement} để xác định tác vụ rà soát có hoàn tất hay không; kết quả lần chạy này không thay thế bảng phát hiện mà giao diện đã nhận từ preflight. Backend không dùng trạng thái \textit{block} làm điều kiện từ chối. Chỉ khi audit không thể hoàn tất, backend mới trả mã \CodeBreak{review\_audit\_failed}; Phản biện viên có thể xác nhận gửi mà không có kết quả audit, và hệ thống lưu sự kiện \CodeBreak{submit\_override\_after\_audit\_failure} kèm lý do.

\subsubsection{Chair Decision Copilot}

Chair Decision Copilot nhận gói bằng chứng gồm điểm số, bản phản biện, phản hồi của Tác giả, nội dung thảo luận và trạng thái liên quan. Hệ thống tạo fingerprint cho gói dữ liệu và từng thành phần, dùng lại bản tổng hợp khi còn hợp lệ hoặc đánh dấu \textit{stale} kèm lý do khi bằng chứng thay đổi. Đầu ra tổ chức điểm đồng thuận, bất đồng, vấn đề còn mở và ghi chú nháp để Chủ tọa đối chiếu.

Luồng không tạo khuyến nghị chấp nhận hoặc từ chối. Chủ tọa phải đọc dữ liệu gốc và tự ghi nhận quyết định trong backend nghiệp vụ. Việc tách bản tổng hợp khỏi quyết định cuối cùng giúp bảo toàn thẩm quyền, nhưng không loại bỏ hoàn toàn ảnh hưởng của cách AI lựa chọn hoặc trình bày bằng chứng; phạm vi này được nêu như một giới hạn khi diễn giải kết quả đánh giá.

\subsubsection{Chatbot Agent của nền tảng}

Chatbot Agent hỗ trợ ba vai trò tra cứu hướng dẫn và dữ liệu trạng thái của hệ thống. Công cụ xác định liệu câu hỏi có cần dữ liệu nghiệp vụ; nếu có, nó tạo yêu cầu gồm tài nguyên, trường, điều kiện lọc, phép tổng hợp và giới hạn số dòng. Backend xác thực người dùng và dịch vụ, kiểm tra danh mục tài nguyên cùng RBAC rồi mới trả dữ liệu.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}[
      ...
      \node[guard] (backend) at (9.0,-3.3) {Backend query endpoint\\xác thực người dùng và dịch vụ; kiểm tra tài nguyên, trường dữ liệu và RBAC};
...
Bảng~\ref{tab:ch3-ai-controls-cut} tổng hợp sáu nhóm kiểm soát được áp dụng tùy theo hợp đồng của từng luồng. Mức triển khai không đồng nhất: một số luồng dùng mô hình Pydantic để kiểm tra đầu ra, trong khi các luồng khác chuẩn hóa bằng mã xử lý riêng hoặc kiểm tra tại nơi áp dụng kết quả. Vì vậy, báo cáo chỉ gán cơ chế cho luồng có bằng chứng triển khai tương ứng.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|P{0.27\textwidth}|Y|}
    ...
    Kiểm tra cấu trúc & Kiểm tra lược đồ hoặc chuẩn hóa trường theo hợp đồng của từng luồng trước khi hiển thị hoặc áp dụng \\ \hline
    Fingerprint và trạng thái hiệu lực & Reviewer Initial Analysis và Chair Decision Copilot nhận diện kết quả \textit{stale}; các luồng khác lưu dữ liệu truy vết theo hợp đồng riêng \\ \hline
    Mã băm chính sách & Submission Gating ghi nhận cấu hình được dùng cho mỗi lần kiểm tra \\ \hline
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=0.82\textwidth}
    \begin{tikzpicture}[node distance=12mm and 18mm, >=Latex]
      ...
      \node[prismNode] (schema) at (0,-9.2) {Kiểm tra theo hợp đồng luồng};
      ...
      \draw[prismArrow] (workflow) -- node[prismEdgeLabel, pos=0.75] {tool result} (redisTemp);
...
Hệ thống dùng API Semantic Scholar để bổ sung hồ sơ và quan hệ đồng tác giả phục vụ đối sánh và phát hiện xung đột lợi ích \cite{ch3ref16}. Dữ liệu này chỉ là nguồn hỗ trợ: hồ sơ có thể thiếu, sai hoặc chưa đồng bộ, nên hệ thống lưu nguồn cùng bằng chứng và để Chủ tọa kiểm tra trước khi sử dụng trong phân công.

\subsection{Ưu điểm và giới hạn của các luồng xử lý AI}

Các luồng AI giảm thao tác nhập lại, hỗ trợ định hướng đọc, rà soát bản nháp và tổ chức nhiều nguồn bằng chứng. Giá trị này phụ thuộc vào việc người dùng hiểu đúng phạm vi hỗ trợ và có khả năng kiểm tra dữ liệu gốc. AI không bảo đảm bản thảo đạt yêu cầu học thuật, không thay thế việc đọc bài và không chịu trách nhiệm về quyết định chấp nhận hoặc từ chối.

Các giới hạn chính gồm chất lượng tệp đầu vào, nguy cơ tạo nhận định thiếu căn cứ, giới hạn độ dài và tần suất gọi dịch vụ, rủi ro khi gửi dữ liệu đến nhà cung cấp bên ngoài và khả năng thay đổi chất lượng giữa các lĩnh vực. Khả năng tiếp tục nghiệp vụ khi AI không khả dụng mới được đáp ứng một phần: Tác giả vẫn có thể nhập bản nháp, Phản biện viên vẫn có thể đọc và viết phản biện, còn Chủ tọa vẫn có thể đối chiếu bằng chứng gốc; riêng thao tác công bố bài hiện vẫn cần một kết quả Submission Gating hợp lệ. Đề tài chưa kiểm toán vòng đời dữ liệu của nhà cung cấp, chưa đo thiên lệch tự động hóa và chưa thử nghiệm chèn lỗi đầu cuối cho toàn bộ sáu luồng; vì vậy, báo cáo không suy diễn các cơ chế kiểm soát thành bảo đảm an toàn hoặc khả năng duy trì nghiệp vụ toàn diện.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tikzpicture}[
      ...
      \draw[prismArrow] (backend) -- (ai);
      \draw[prismDashedArrow, bend left=22] (ai) to node[prismEdgeLabel] {backend query endpoint} (backend);
      \draw[prismArrow] (backend) -- (postgres);
      \draw[prismArrow] (backend) -- (neo4j);
    ...
    \end{tikzpicture}
  \end{adjustbox}
  \caption{Topology được định nghĩa trong cấu hình production với ranh giới VPS, mạng \textit{app} và mạng \textit{data}}
...
Docker Compose định nghĩa container image (gói phát hành chứa môi trường chạy), biến môi trường, volume, kiểm tra trạng thái, mạng và chính sách khởi động lại. Next.js, backend và dịch vụ AI được xây dựng thành ba image của dự án; dịch vụ \CodeBreak{frontend} dùng image Caddy công khai làm reverse proxy. Mỗi image của dự án chứa môi trường chạy và phần phụ thuộc của dịch vụ tương ứng. Thẻ commit SHA cho phép truy ngược ba image này đến phiên bản mã nguồn mà quy trình đã xây dựng. Cơ chế đó không tự bảo đảm build có thể tái tạo hoàn toàn, vì kết quả còn phụ thuộc phiên bản image nền, gói phụ thuộc và hạ tầng xây dựng.
...
\begin{figure}[H]
  ...
  \begin{adjustbox}{max width=0.85\textwidth}
    \begin{tikzpicture}[node distance=12mm and 18mm, >=Latex]
      ...
      \node[prismNode] (ghcr) at (0,-5.0) {Push image lên GHCR};
...
Quy trình giữ máy chủ ở vai trò triển khai image đã được xây dựng, thay vì dùng máy chủ làm môi trường build. Việc gắn image với commit SHA cung cấp dấu vết từ bản phát hành về mã nguồn. Khả năng tạo lại đúng cùng gói phát hành, rollback và độ tin cậy dài hạn cần cơ chế cùng bằng chứng vận hành riêng.

\subsection{Cô lập mạng, volume dữ liệu và bảo vệ giá trị bí mật}

PostgreSQL và Redis chỉ tham gia mạng dữ liệu; backend và dịch vụ AI tham gia mạng ứng dụng cùng mạng dữ liệu để phục vụ yêu cầu nội bộ. Neo4j không mở cổng công khai trong cấu hình chính thức, còn Caddy chỉ tham gia mạng ứng dụng. Các volume riêng duy trì dữ liệu PostgreSQL, Redis, Neo4j, tệp tải lên và trạng thái Caddy khi container được tạo lại. Volume không phải bản sao lưu và cấu hình hiện chưa chứng minh khả năng khôi phục sau mất dữ liệu hoặc hỏng máy chủ.

Tệp \CodeBreak{.env.production} được đặt quyền truy cập hạn chế trên máy chủ. Đây là biện pháp bảo vệ cơ bản đối với mật khẩu, mã xác thực và khóa gọi dịch vụ bên ngoài. Phạm vi vận hành được mô tả ở đây gồm cô lập mạng, lưu dữ liệu bền vững và truy vết bản phát hành; kiểm toán bảo mật, giám sát dài hạn, sao lưu--phục hồi và rollback cần được đánh giá bằng quy trình riêng.
...
Các ranh giới được mô tả theo hành vi thực tế của từng luồng. Thuật toán tự động loại các cặp có xung đột lợi ích, còn luồng thêm đề xuất thủ công cảnh báo để Chủ tọa xử lý. Review Quality Auditor làm nổi bật phát hiện nghiêm trọng nhưng không ngăn gửi; chỉ lỗi thực thi audit mới yêu cầu Phản biện viên xác nhận tiếp tục và được ghi vào nhật ký. Chatbot Agent nhận hội thoại qua Next.js nhưng truy vấn dữ liệu nghiệp vụ qua backend và RBAC. Cách phân định này cho thấy rõ điểm hệ thống cưỡng chế, điểm chỉ cung cấp cảnh báo và điểm cần quyết định của con người.

Bảng~\ref{tab:ch3-design-evidence-cut} phân biệt thành phần đã triển khai với phép đo thực tế ở Chương 4. Nhờ đó, độ chính xác của siêu dữ liệu, tính hợp lệ của chuyên đề, kiểm tra luật, chất lượng đầu ra AI và hiệu năng thuật toán được diễn giải theo đúng dữ liệu đánh giá tương ứng.
...
\begin{longtable}{|P{0.27\textwidth}|P{0.32\textwidth}|P{0.33\textwidth}|}
  ...
  Phân công tự động có điểm số giải thích được & Jaccard, thuật toán đối sánh GreedyMatcher và xác nhận của Chủ tọa & Đo chất lượng và thời gian xử lý trên tập thử; thuật toán chưa có quy tắc phân xử khi bằng điểm và chưa nhận tải lịch sử \\ \hline
  COI được cưỡng chế trong thuật toán tự động & Kiểm tra tác giả, khai báo và đồ thị đồng tác giả & Giữ COI trong lượt dự phòng; luồng thêm đề xuất thủ công chỉ cảnh báo và chưa lưu bản ghi kiểm toán riêng gồm người xác nhận, thời điểm và lý do; độ phủ Neo4j phụ thuộc truy vấn thành công \\ \hline
  ...
  RQA hỗ trợ rà soát nhưng không tước quyền gửi & Nhãn \textit{pass}/\textit{warn}/\textit{block}; xác nhận khi audit thất bại & Chương 4 đo mức bám nguồn và mức nhiễu của phát hiện; không dùng phân bố nhãn để suy ra hành vi chặn khi vận hành \\ \hline
  Chatbot giới hạn dữ liệu theo quyền & Backend query endpoint, mã xác thực dịch vụ, danh mục tài nguyên và RBAC & Kịch bản hội thoại kiểm tra ranh giới quyền trong phạm vi bộ thử \\ \hline
  ...
  Nghiệp vụ tiếp tục khi dịch vụ AI không khả dụng & Đường thao tác thủ công ở Autofill, RIA, RQA và Decision Copilot; xử lý lỗi theo từng luồng & Đáp ứng một phần; chưa thử nghiệm chèn lỗi đầu cuối cho toàn bộ dịch vụ và thao tác công bố bài vẫn cần kết quả Submission Gating hợp lệ \\ \hline
  Bản phát hành có thể truy vết đến mã nguồn & Docker Compose, image theo commit SHA, Caddy và GitHub Actions & Cấu hình thể hiện cơ chế đóng gói và truy vết; chưa có bằng chứng runtime để khẳng định VPS, HTTPS và quyền tệp bí mật đang vận hành đúng, đồng thời chưa chứng minh build tái tạo hoàn toàn, rollback hoặc sao lưu-phục hồi \\ \hline
\end{longtable}
\endgroup
