...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[>=Latex,
    block/.style={draw=black!65, rounded corners, align=center, text width=3.2cm, minimum height=1cm, fill=gray!8},
    arrow/.style={-{Latex[length=2mm]}, thick}
  ]
    \node[block] (data) at (0,4) {Bài nộp và\\ ngữ cảnh nguồn};
    \node[block] (worker) at (0,1) {Bộ thực thi\\ luồng xử lý};
    \node[block] (outputs) at (0,-2) {Gói đầu ra\\ và điểm kiểm tra};
    \node[block] (direct) at (4.8,-5) {Đối chiếu trực tiếp\\ Autofill / Gating};
    \node[block] (tca) at (-4.8,-5) {TCA\ cho đầu ra văn bản};
    \node[block] (chat) at (0,-5) {Chấm theo kịch bản\\ Chatbot Agent};
