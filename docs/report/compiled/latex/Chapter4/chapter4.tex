...
Chương 3 đã trình bày ConferenceSpace theo ba lớp trách nhiệm: nghiệp vụ cốt lõi, thuật toán tất định có thể kiểm chứng và AI hỗ trợ. Chương này trả lời câu hỏi tổng quát: trong điều kiện thử nghiệm, mỗi lớp thực hiện trách nhiệm được giao đến mức nào? Các nhóm bằng chứng được phân tách theo bản chất của từng tác vụ thay vì dùng một thước đo chung:
...
\begin{longtable}{|C{0.313\textwidth}|P{0.313\textwidth}|P{0.313\textwidth}|}
  ...
  Thuật toán tất định, có thể kiểm chứng      &      Đối sánh phản biện viên, phát hiện xung đột lợi ích và các chỉ số chất lượng xếp hạng, phân công      &      Cung cấp bằng chứng rằng cơ chế tất định và có thể kiểm chứng xử lý các tác vụ đòi hỏi kết quả ổn định, có thể tái tạo và giải thích, qua đó tránh phụ thuộc vào tính ngẫu nhiên của mô hình AI tạo sinh \\ \hline
...
Nhóm thực hiện từng nhóm thực nghiệm trong môi trường phù hợp với loại hành vi cần đo. Bảng 4.2 tóm tắt môi trường, công cụ và tư liệu chính của từng nhóm.
...
\begin{longtable}{|P{0.235\textwidth}|P{0.235\textwidth}|P{0.235\textwidth}|P{0.235\textwidth}|}
  ...
  \LongTableFooterBorder
  \endfoot
  Kiểm thử hiệu năng HTTP của backend     &     Backend gồm PostgreSQL, Neo4j, Redis và bộ chứa API; k6 tạo tải HTTP     &     Bản tóm tắt JSON theo kịch bản, nhật ký tài nguyên và bản tóm tắt mức sử dụng tài nguyên     &     Đo độ trễ, thông lượng, tỷ lệ lỗi yêu cầu và mức tài nguyên tiêu thụ của lớp nghiệp vụ \\ \hline
  Kiểm thử vi mô bằng Go      &      Bộ kiểm thử Go chạy trực tiếp trên mã thuật toán      &      Tệp \textit{micro.txt} chứa thời gian mỗi thao tác (ns/op), bộ nhớ cấp phát mỗi thao tác (B/op) và số lần cấp phát bộ nhớ mỗi thao tác (allocs/op)      &      Tách thời gian xử lý và mức sử dụng bộ nhớ của thuật toán khỏi chi phí HTTP, cơ sở dữ liệu và mạng \\ \hline
...
\begin{longtable}{|P{0.20\textwidth}|P{0.24\textwidth}|C{0.14\textwidth}|P{0.20\textwidth}|C{0.12\textwidth}|}
  ...
  Kiểm thử vi mô bằng Go      &      Đối sánh phản biện viên và phát hiện xung đột lợi ích tiêu tốn bao nhiêu thời gian xử lý và bộ nhớ, không bao gồm chi phí HTTP, cơ sở dữ liệu và mạng?      &      Nhiều quy mô dữ liệu thử nghiệm thuật toán      &      \textit{ns/op}, \textit{B/op}, \textit{allocs/op}      &      Mục 4.4.1 \\ \hline
  ...
  Phép chấm TCA thăm dò     &     Liệu đầu ra tự luận có bám bằng chứng nguồn hay không?     &     1.097 gói kết quả     &     Truthfulness, Coverage, Additionality, Validity     &     Mục 4.5 \\ \hline
...
\textbf{Đầu vào.} Trước khi đo, nhóm khởi tạo hệ thống bằng bộ dữ liệu tổng hợp gồm 300 hội nghị, 50 bài nộp cho mỗi hội nghị và 30 phản biện viên cho mỗi hội nghị, tương ứng với 15.000 bài nộp và 9.000 quan hệ phản biện viên--hội nghị. Môi trường đo gồm bộ chứa API, PostgreSQL, Neo4j và Redis. Cấu hình tải mặc định sử dụng 20 người dùng ảo trong 30 giây cho mỗi kịch bản k6.
...
\begin{itemize}
  ...
  \item \textbf{Đầu ra kỳ vọng:} mỗi mệnh đề có trạng thái bám nguồn có thể truy ngược; mỗi luồng xử lý có các điểm Truthfulness, Coverage, Additionality, Validity, tỷ lệ vừa bám nguồn vừa hợp lệ hoặc tỷ lệ rủi ro cao tương ứng. Đây là điểm thăm dò tự động vì quy trình chưa được hiệu chuẩn bằng mẫu gán nhãn chuyên gia, chưa báo cáo độ tin cậy của bước trích mệnh đề và chưa có ngưỡng quyết định được xác lập trước.
  \item \textbf{Đầu ra thực tế và chỉ số:} JSONL theo từng bài và các bảng tại Mục 4.5.5--4.5.7.
\end{itemize}

Hai bước này liên kết trực tiếp: bộ thực thi tạo tập đầu ra thực nghiệm, còn TCA chỉ đánh giá tập đó mà không tạo lại đầu ra. Do đó, độ trễ thấp ở bộ thực thi chưa đủ để kết luận về chất lượng học thuật; chỉ số Truthfulness cao ở TCA cũng không có nghĩa là hệ thống có thể thay thế Phản biện viên hoặc Chủ tọa.
...
Nhìn tổng thể, các kịch bản trên tạo thành một chuỗi bằng chứng theo thứ tự. Trước hết, nhóm đo hiệu năng của các đường nghiệp vụ đã chọn. Tiếp theo, nhóm đo thời gian xử lý và hành vi xếp hạng, phân công của thuật toán tất định trên bộ thử. Sau đó, nhóm đánh giá từng luồng AI trên tác vụ cụ thể. Cuối cùng, phản hồi của người dùng bổ sung góc nhìn về trải nghiệm. Các mục từ 4.3 đến 4.7 báo cáo kết quả theo phương pháp đã xác định tại đây.

\subsection{Phạm vi và giới hạn của thực nghiệm}

Thực nghiệm hiện tại cung cấp bằng chứng cho ba nhóm kết luận. Thứ nhất, ba endpoint đã chọn của backend và mã thuật toán được kiểm thử có thời gian xử lý thấp trong cấu hình thử nghiệm. Thứ hai, các luồng AI có thể tạo đầu ra trên hơn một nghìn bài nộp; một phần đầu ra được đánh giá bằng chỉ số mệnh đề--bằng chứng. Thứ ba, một số luồng đã có bằng chứng vận hành hoặc chuẩn đầu ra rõ ràng, chẳng hạn phép kiểm tra luật của Submission Gating, khả năng trích xuất siêu dữ liệu của Submission Autofill và khả năng tuân thủ ranh giới quyền truy cập của Chatbot Agent.

Tuy nhiên, thực nghiệm không cho thấy AI có thể thay thế Phản biện viên hoặc Chủ tọa. TCA chỉ cung cấp các chỉ số tự động gián tiếp về mức độ bám bằng chứng và không thay thế đánh giá đầy đủ của chuyên gia. Gợi ý chuyên đề trong Submission Autofill chưa có nhãn chuyên gia để đánh giá độ chính xác khi chọn chuyên đề. Phần cảnh báo mềm của Submission Gating cũng chưa được con người rà soát từng cảnh báo nội dung. Chair Decision Copilot chưa được đối chiếu với quyết định chấp nhận hoặc từ chối của Chủ tọa, nên không được trình bày như một bộ phân loại quyết định. Chatbot Agent mới được đánh giá trên 40 hội thoại; quy mô này đủ cho nhận định ban đầu theo kịch bản nhưng chưa đủ để kết luận về độ ổn định dài hạn.
...
\item \textbf{Đánh giá dựa trên mệnh đề và bằng chứng:} TCA đánh giá Reviewer Initial Analysis, Review Quality Auditor và Chair Decision Copilot nhằm ước lượng mức độ phù hợp giữa nhận định do hệ thống tạo ra và bằng chứng trong bài báo, bản phản biện hoặc bản tổng hợp. Độ phủ đo mức trùng khớp với nội dung tham chiếu do con người tạo. Khả năng bổ sung đo tỷ lệ mệnh đề được bộ chấm xem là có căn cứ nhưng không trùng trực tiếp với tham chiếu; chỉ số này không tự chứng minh thông tin mới hữu ích về mặt học thuật.

\item \textbf{Đánh giá theo tình huống sử dụng:} Nhóm kiểm tra Chatbot Agent qua các đoạn hội thoại mô phỏng theo ba vai trò: Tác giả, Phản biện viên và Chủ tọa. Nhóm đánh giá thủ công kết quả theo ba tiêu chí: câu trả lời có giải quyết đúng yêu cầu, có dựa trên dữ liệu được cung cấp và có tuân thủ phạm vi quyền truy cập của từng vai trò hay không.
\end{itemize}

Tư liệu thô gồm các tệp JSON, JSONL và CSV của bộ thực thi luồng xử lý, TCA, Submission Gating, gợi ý chuyên đề trong Submission Autofill và Chatbot Agent. Các đầu ra này cho phép truy ngược từ số liệu tổng hợp đến từng tác vụ, phản hồi, cảnh báo hoặc phát hiện khi cần kiểm tra lại. Các bảng trong báo cáo sử dụng số liệu từ báo cáo thử nghiệm đã tổng hợp; đầu ra thô cung cấp bằng chứng để truy xuất và kiểm tra nguồn gốc dữ liệu.

\par\noindent\rule{\textwidth}{0.4pt}\par

\section{Đánh giá lớp nghiệp vụ cốt lõi}

Phần này đo hiệu năng vận hành của một số đường xử lý thuộc lớp nghiệp vụ cốt lõi: đăng nhập, truy vấn danh sách, đối sánh phản biện viên và kiểm tra xung đột lợi ích. Các kịch bản không gọi dịch vụ AI, nhưng việc tách đường gọi không chứng minh toàn bộ vòng đời vẫn hoạt động khi AI ngừng hoạt động; nhận định đó cần thử nghiệm chèn lỗi đầu cuối riêng.

\subsection{Kịch bản tải HTTP}

Bộ kiểm thử hiệu năng của backend kết hợp hai lớp đo bổ sung cho nhau. k6 đo độ trễ và thông lượng đầu cuối qua các endpoint HTTP thực tế, qua đó phản ánh thời gian xử lý sau khi yêu cầu đi qua API, cơ sở dữ liệu và tầng trung gian. Kiểm thử vi mô bằng Go đo trực tiếp thời gian xử lý và mức sử dụng bộ nhớ của thuật toán trong chương trình, không bao gồm chi phí HTTP, cơ sở dữ liệu và mạng. Ba kịch bản tải HTTP được thực thi trên tập dữ liệu gồm 300 hội nghị, 15.000 bài nộp và 9.000 phản biện viên; giai đoạn khởi tạo dữ liệu không phát sinh lỗi:

\begin{itemize}
  \item \textbf{Nhóm truy vấn đọc:} sau bước đăng nhập trong giai đoạn thiết lập, mỗi vòng lặp lần lượt liệt kê hội nghị, liệt kê bài nộp và tìm kiếm người dùng; kịch bản không tạo, cập nhật hoặc xóa bản ghi.
  \item \textbf{Gợi ý phản biện viên:} gọi endpoint \CodeBreak{reviewer-suggestions}; kịch bản không tự động lưu phân công.
  \item \textbf{Xung đột lợi ích:} gọi endpoint kiểm tra xung đột lợi ích, bao gồm cả truy vấn đồ thị đồng tác giả trên Neo4j.
\end{itemize}
...
Kết quả kiểm thử vi mô bằng Go ghi nhận thời gian xử lý từ 131 \(\mu\)s đến 56 ms tùy quy mô dữ liệu, không bao gồm HTTP, cơ sở dữ liệu, mạng hoặc thời gian hiển thị. Phép đo cho thấy chi phí tính toán của mã thuật toán thấp ở các quy mô đã thử, nhưng không tự chứng minh thời gian phản hồi tức thời của toàn hệ thống.


\begin{PrismLongTable}
\begin{longtable}{|C{0.235\textwidth}|P{0.235\textwidth}|P{0.235\textwidth}|P{0.235\textwidth}|}
  \caption{Kết quả kiểm thử vi mô bằng Go theo thuật toán}\LongTableCaptionSpace
  \hline
...
Phần này đánh giá hành vi xếp hạng và phân công của thuật toán đối sánh phản biện viên trên tập dữ liệu tổng hợp lưu tại \CodeBreak{testdata/s2\_snapshot.json}. Tập dữ liệu mô phỏng lược đồ (schema) dữ liệu Semantic Scholar; nhóm chạy phép đánh giá cục bộ mà không gọi API. Cách thực hiện này cố định đầu vào và cho phép chạy lại cơ chế xếp hạng tất định, nhưng kết quả không phải bằng chứng trên dữ liệu Semantic Scholar thực.

\subsubsection{Dữ liệu và phương pháp nhãn tham chiếu}

Tập dữ liệu được lưu cùng mã nguồn là dữ liệu tổng hợp vì tại thời điểm tạo dữ liệu, nhóm không có khóa truy cập Semantic Scholar khả dụng. Tập này gồm 60 hồ sơ tác giả và 2.565 bản ghi bài báo, được tổ chức theo lược đồ (schema) dự kiến của bộ thu thập dữ liệu. Vì vậy, các chủ đề và quan hệ tác giả--bài báo phản ánh dữ liệu mô phỏng, không phải mẫu quan sát từ cộng đồng nghiên cứu. Báo cáo giữ nguyên quy mô và kết quả để đánh giá khả năng tái tạo thuật toán trên bộ thử này, đồng thời giới hạn kết luận ở phép so sánh nội bộ giữa các phương pháp trên dữ liệu tổng hợp.
...
\textbf{Giới hạn biểu diễn và dữ liệu:} Điểm Jaccard trung bình 0,011 cho thấy tín hiệu giao từ khóa thưa trên bộ tổng hợp mở rộng. Kết quả có thể xuất phát từ phương pháp tạo dữ liệu, mức độ chuẩn hóa thuật ngữ hoặc giới hạn của cách biểu diễn bằng tập từ khóa; thử nghiệm hiện tại chưa tách riêng các nguyên nhân. Các giá trị MRR 0,392 và điểm trung bình 0,011 chỉ là đường cơ sở tái tạo được trên bộ thử này để so sánh với phương pháp khác trong tương lai.

\textbf{Nhãn tham chiếu gián tiếp:} Phương pháp leave-one-out dùng tác giả gốc làm nhãn truy hồi, trong khi chính tác giả đó phải bị loại khi chọn phản biện viên. Vì vậy, chỉ số đo khả năng khôi phục quan hệ tác giả--chủ đề đã mã hóa trong dữ liệu, không đo chất lượng đối sánh phản biện viên hoặc chất lượng phản biện sau phân công.

\textbf{Quy mô dữ liệu:} 60 hồ sơ tác giả cho phép so sánh thuật toán và đường cơ sở trong cùng bộ thử, nhưng quá nhỏ và quá đơn giản để đại diện cho hội nghị thật. Kết quả có thể dùng để phát hiện sơ bộ đánh đổi giữa điểm từ khóa, độ phủ và tải; chưa đủ để suy luận yêu cầu thuật toán ở quy mô lớn nếu chưa có phép đo độ phức tạp và dữ liệu vận hành tương ứng.
...
Về hiệu năng, Bảng 4.5 cho thấy kiểm thử vi mô của hai bộ phát hiện \textit{self-author} và xung đột đã khai báo tốn 14,9 \(\mu\)s đến 653 \(\mu\)s tùy quy mô dữ liệu. Phép đo không bao gồm bộ phát hiện quan hệ đồng tác giả trên Neo4j: kiểm thử đồ thị riêng chỉ chạy khi có môi trường Neo4j đã gieo dữ liệu và không có kết quả được dùng trong bảng. Vì vậy, số liệu không đại diện cho thời gian của toàn bộ cơ chế xung đột lợi ích đa tầng.

\subsection{Giới hạn hiện tại của phép đánh giá đối sánh}

Dữ liệu thực nghiệm hiện tại không chỉ đo thời gian chạy mà còn gồm phép leave-one-out và các chỉ số nội tại của phân công trên dữ liệu tổng hợp. Tuy nhiên, Hit@k, MRR và nDCG đo khả năng truy hồi hồ sơ tác giả gốc; độ phủ, độ đồng đều và tỷ lệ dùng lượt dự phòng mô tả hành vi của thuật toán gán. Chúng chưa phải bằng chứng về mức phù hợp chuyên môn do Chủ tọa xác nhận. Vì vậy, chương chỉ kết luận thuật toán tái tạo được một tín hiệu giao từ khóa và bộc lộ rõ đánh đổi độ phủ--điểm số trên bộ thử; chưa kết luận về mức chấp nhận đề xuất hoặc chất lượng phản biện thực tế.

Tương tự, dữ liệu hiện có chưa lượng hóa được số lượng quan hệ xung đột lợi ích ẩn mà lớp phân tích đồ thị phát hiện thêm so với chỉ dựa vào khai báo thủ công --- chỉ số quan trọng để đánh giá giá trị gia tăng thực sự của lớp Neo4j. Đây là những hạn chế được ghi nhận rõ ràng để bổ sung ở giai đoạn thực nghiệm tiếp theo.
...
Các luồng AI của ConferenceSpace tạo ra những loại đầu ra khác nhau, nên chương không dùng một chỉ số chung như ``độ chính xác'' cho toàn bộ lớp AI. Submission Autofill tạo siêu dữ liệu có dữ liệu tham chiếu rõ ràng. Reviewer Initial Analysis, Review Quality Auditor và Chair Decision Copilot tạo đầu ra tự luận, cần được kiểm tra theo quan hệ mệnh đề--bằng chứng. Chatbot Agent tương tác với các công cụ nội bộ, nên phép đánh giá dựa trên kết quả hội thoại và khả năng tuân thủ quyền truy cập.

Thử nghiệm gồm hai lớp chính. Lớp thứ nhất là bộ thực thi luồng xử lý, dùng để chạy các luồng đã triển khai trên tập dữ liệu, đồng thời lưu đầu ra, thời gian xử lý, số token tiêu thụ, trạng thái hoàn tất và các chỉ số đối sánh trực tiếp khi có thể tính được. Lớp thứ hai là bộ thử nghiệm TCA, đọc lại đầu ra đã lưu để đánh giá độ trung thực, độ phủ và khả năng bổ sung. Việc tách bộ tạo đầu ra khỏi bộ đánh giá giúp báo cáo không đánh đồng khả năng tạo đầu ra với chỉ số gián tiếp về mức độ bám bằng chứng.

TCA gồm ba chỉ số: (1) độ trung thực (Truthfulness), là tỷ lệ mệnh đề được bộ chấm NLI xác định có bằng chứng hỗ trợ; (2) độ phủ (Coverage), là mức độ trùng khớp với đầu ra tham chiếu do con người tạo; và (3) khả năng bổ sung (Additionality), là tỷ lệ mệnh đề được xem là có căn cứ nhưng không trùng trực tiếp với tham chiếu. Khả năng bổ sung không đo trực tiếp tính mới hoặc mức độ hữu ích về học thuật. Độ phủ thấp cũng không mặc nhiên là lỗi vì nhận xét phản biện và nhận xét tổng hợp chỉ ghi lại một phần nội dung đã được cân nhắc; ngược lại, báo cáo cũng không mặc nhiên xem mọi phần nằm ngoài tham chiếu là giá trị bổ sung.

Nguyên tắc quan trọng nhất khi diễn giải các kết quả dưới đây là giới hạn kết luận trong phạm vi bằng chứng. Mức độ bám bằng chứng cao không cho thấy AI có thể thay thế phản biện viên. Độ trễ thấp không cho thấy đầu ra đúng. Chuyên đề hợp lệ không đồng nghĩa với chuyên đề được chuyên gia đánh giá là phù hợp. Bản tóm tắt quyết định có độ trung thực cao cũng không cho thấy hệ thống dự đoán đúng quyết định chấp nhận hoặc từ chối.
...
\begin{longtable}{|P{0.188\textwidth}|P{0.188\textwidth}|P{0.188\textwidth}|P{0.188\textwidth}|P{0.188\textwidth}|}
  ...
  Reviewer Initial Analysis      &      Bộ thực thi luồng xử lý và TCA      &      1.127 lượt thực thi; 1.097 gói TCA      &      Tỷ lệ trích dẫn bám nguồn, tỷ lệ bịa đặt, độ trung thực của điểm chú ý, độ phủ và khả năng bổ sung      &      Mức độ bám nguồn theo bộ chấm và rủi ro của điểm chú ý; không thay thế đánh giá chuyên gia \\ \hline
  ...
  Chair Decision Copilot      &      Bộ thực thi luồng xử lý và TCA      &      1.127 lượt thực thi; 1.097 gói TCA      &      Độ trung thực của cơ sở bằng chứng và bản đồ bất đồng, khả năng bổ sung, tỷ lệ rủi ro cao      &      Mức độ bám nguồn của bản tổng hợp; không đo độ chính xác của quyết định chấp nhận hoặc từ chối hay thiên lệch tự động hóa \\ \hline
...
Theo bộ chấm tự động, tỷ lệ trích dẫn được phân loại là bám nguồn đạt 96,22\% và tỷ lệ bị phân loại là không bám nguồn trung bình là 3,78\%. Kết quả cho thấy bộ chấm nhận diện phần lớn trích dẫn là có căn cứ trong nguồn, nhưng chưa phải bằng chứng mạnh về tính đúng đắn vì phép chấm chưa được hiệu chuẩn với nhãn chuyên gia. Độ trung thực của điểm chú ý thấp hơn, đạt 69,86\%, vì điểm cần chú ý thường là diễn giải hoặc gợi ý điều cần chú ý, khó kiểm chứng trực tiếp hơn trích dẫn.

Độ phủ thấp (4,49\%) không có nghĩa là luồng kém, vì nhận xét phản biện của con người thường chỉ ghi lại những điểm nổi bật, không bao phủ toàn bộ các nhận xét mà người đọc đã cân nhắc. Do đó, so sánh trực tiếp độ phủ sẽ không phản ánh đúng chất lượng của bản định hướng. Reviewer Initial Analysis được tạo trước khi phản biện viên viết nhận xét, còn nhận xét của con người thường chỉ ghi một số vấn đề nổi bật sau khi đọc sâu. Khả năng bổ sung 92,23\% cho biết bộ chấm xếp phần lớn mệnh đề có căn cứ vào nhóm không trùng trực tiếp với nhận xét tham chiếu; chỉ số này không xác nhận các mệnh đề đó mới hoặc hữu ích đối với phản biện viên. Khi đối chiếu với UAT cỡ mẫu nhỏ, kết quả gợi ý luồng có thể phục vụ vai trò định hướng đọc, nhưng mọi điểm cần chú ý vẫn phải được phản biện viên kiểm tra trên bài gốc.
...
Theo bộ chấm TCA, cơ sở bằng chứng đạt 87,34\% và bản đồ bất đồng đạt 87,11\% về độ trung thực. Khả năng bổ sung 91,63\% cho biết nhiều mệnh đề được chấm là có căn cứ nhưng không trùng trực tiếp với bản tổng hợp nhận xét; kết quả không tự chứng minh các mệnh đề này hữu ích hoặc nên ảnh hưởng quyết định. Độ phủ thấp cũng không thể được xem là lỗi hay lợi thế nếu chưa có Chủ tọa đánh giá độ đầy đủ của bản tổng hợp trong bối cảnh ra quyết định.
...
Các luồng AI cung cấp bằng chứng ban đầu cho một số tác vụ hỗ trợ, nhưng chưa đủ để kết luận cách tích hợp hiện tại đã kiểm soát đầy đủ mọi ranh giới trách nhiệm. Các giới hạn cần giữ xuyên suốt báo cáo gồm:
...
\begin{itemize}
  ...
  \item \textbf{TCA là chỉ số gián tiếp, không thay thế chuyên gia.} Độ trung thực, độ phủ và khả năng bổ sung giúp đo mức độ bám bằng chứng và xu hướng bổ sung thông tin, nhưng không thể xác định toàn bộ chất lượng học thuật của một nhận xét.
  ...
  \item \textbf{Chatbot Agent chưa đủ ổn định để triển khai rộng.} Nhóm cần giảm số lần gọi công cụ thất bại và diễn đạt rõ ranh giới quyền truy cập trước khi dùng trong trình diễn hoặc môi trường thực tế.
\end{itemize}

Những giới hạn này xác định ranh giới triển khai của lớp AI: Autofill và kiểm tra luật có bằng chứng trực tiếp hơn, còn các luồng tạo nội dung gần hoạt động phản biện hoặc quyết định chỉ nên được xem là thử nghiệm. Riêng Quality Auditor chưa thể được xem là công cụ hỗ trợ có kiểm soát khi các phát hiện theo xác suất vẫn có thể chặn thao tác mà người dùng không thể ghi đè.
...
\begin{longtable}{|P{0.152\textwidth}|C{0.152\textwidth}|C{0.152\textwidth}|C{0.152\textwidth}|C{0.152\textwidth}|P{0.152\textwidth}|}
  \caption{Độ trễ và lượng token theo luồng xử lý AI}\LongTableCaptionSpace
  \hline
...
Kiến trúc gồm bộ điều phối và các worker lưu gói kết quả riêng cho từng luồng AI, tạo điểm kiểm tra theo giai đoạn và cho phép truy ngược đầu ra sau thử nghiệm. Cấu trúc này hỗ trợ cô lập trạng thái theo luồng ở mức thiết kế; chưa có thử nghiệm chèn lỗi đầu cuối để xác nhận hành vi khi dịch vụ AI hết thời gian chờ hoặc ngừng hoạt động.

Các giới hạn chính hiện tại gồm:

\begin{itemize}
  \item \textbf{Các trường hợp có độ trễ cao:} Submission Autofill, Reviewer Initial Analysis, Review Quality Auditor và Chair Decision Copilot đều có trường hợp vượt 100 giây. Hệ thống cần quy định thời gian chờ tối đa, cơ chế thử lại, trạng thái xử lý và chế độ chạy nền cho các luồng dài.
...
Vì vậy, tính khả thi vận hành của ConferenceSpace không nên được trình bày như ``AI chạy tự động từ đầu đến cuối''. Kết luận phù hợp với bằng chứng là hệ thống có nền tảng vận hành để đưa AI vào các điểm hỗ trợ cụ thể, với điều kiện giao diện thể hiện rõ trạng thái, nguồn bằng chứng, mức rủi ro và quyền xác nhận cuối cùng của con người.
...
Khảo sát sử dụng bảng hỏi trực tuyến gồm câu hỏi mở và thang Likert 5 mức. Ba tệp dữ liệu gốc chứa 91 phản hồi có xác nhận đồng thuận và đủ dữ liệu cho các chỉ số chính, gồm 76 phản hồi của Tác giả, 7 phản hồi của Phản biện viên và 8 phản hồi của Chủ tọa. Mẫu được tuyển theo phương pháp thuận tiện và không có nhóm đối chứng. Vai trò Chủ tọa hoặc vị trí tương đương đòi hỏi kinh nghiệm chuyên môn và điều phối; trong khi đó, nguồn lực tuyển mẫu của nhóm còn hạn chế. Vì vậy, nhóm gặp khó khăn khi mời đúng đối tượng và chuyển phiếu dành cho Chủ tọa sang hình thức ẩn danh để giảm rào cản tham gia. Theo quy trình thu thập của nhóm, tám phản hồi này đến từ tám lượt tham gia ẩn danh; do lựa chọn ẩn danh, tệp xuất ghi cùng một địa chỉ đại diện chứ không phải các bản ghi trùng lặp. Tuy nhiên, quy trình không cấp mã riêng cho từng người tham gia, nên tệp xuất không cho phép kiểm chứng độc lập tính duy nhất, danh tính hoặc hồ sơ chuyên môn của mẫu Chủ tọa. Nhóm Tác giả chiếm phần lớn mẫu, còn hai nhóm đòi hỏi kinh nghiệm chuyên môn cao hơn có cỡ mẫu nhỏ. Vì vậy, kết quả chỉ mô tả trải nghiệm của mẫu tham gia, không đại diện cho toàn bộ cộng đồng hội nghị.

Các câu hỏi chính bao gồm: (1) mức độ hài lòng tổng thể và theo chức năng, (2) mức đồng ý rằng AI giúp giảm thao tác thủ công hoặc tiết kiệm thời gian, (3) những khó khăn gặp phải và (4) đề xuất cải thiện. Mục (2) là nhận định trên thang đo, không phải phép đo thời gian thao tác.

Tương tự khảo sát nhu cầu ở Chương 2, việc trực quan hóa thành phần mẫu là cần thiết để người đọc hiểu đúng mức độ khái quát của kết quả. Hình~\ref{fig:ch4-uat-role-distribution} cho thấy nhóm Tác giả chiếm 83,5\% mẫu phân tích, trong khi Phản biện viên và Chủ tọa lần lượt chiếm 7,7\% và 8,8\%. Tám phản hồi Chủ tọa cho phép tổng hợp phân bố ban đầu theo vai trò, nhưng cỡ mẫu nhỏ và quy trình ẩn danh vẫn không cho phép suy rộng thống kê hoặc kiểm chứng độc lập đặc điểm của nhóm này.
...
\begin{longtable}{|P{0.18\textwidth}|P{0.25\textwidth}|P{0.19\textwidth}|P{0.30\textwidth}|}
  ...
  Phát hiện xung đột lợi ích & Kiểm thử vi mô cho ca tự gán và xung đột đã khai báo; không có ca tự gán trong tập phân công & Chưa đo chất lượng xung đột lợi ích đa tầng & Chưa kiểm thử lớp đồ thị hoặc xây dựng tập cặp xung đột do chuyên gia gán nhãn \\ \hline
  ...
  Initial Analysis & Điểm TCA thăm dò trên 1.097 gói; 96,22\% trích dẫn bám nguồn và 69,86\% điểm chú ý trung thực theo bộ chấm & Chỉ có chỉ số gián tiếp & TCA/NLI chưa hiệu chuẩn; đầu ra có thể ảnh hưởng nội dung phản biện; chưa đo mức độ hữu ích, mức độ phụ thuộc hoặc sự phù hợp với chính sách \\ \hline
  ...
  Giảm phụ thuộc vào AI & Đường thao tác thủ công và ranh giới kiến trúc được mô tả & Chưa đo đầu cuối & Chưa có thử nghiệm chèn lỗi khi dịch vụ AI hết thời gian chờ hoặc ngừng hoạt động \\ \hline
...
Đối chiếu với Chương 1 và Chương 2, lớp nghiệp vụ có bằng chứng hiệu năng cho các endpoint đã thử, nhưng chưa có bằng chứng chức năng đầu cuối cho toàn bộ vòng đời. Trên dữ liệu tổng hợp, phương pháp xếp hạng Jaccard truy hồi hồ sơ tác giả gốc tốt hơn phương pháp cơ sở ngẫu nhiên; phép thử chưa đo mức độ phù hợp của phản biện viên. Thủ tục phân công đạt độ phủ đủ hai người ở 65,9\% số bài; 23,3\% số bài phải dùng lượt dự phòng không còn ngưỡng điểm và giới hạn tải tối đa. Với xung đột lợi ích, kiểm thử vi mô chỉ bao phủ ca tự gán và xung đột đã khai báo; chất lượng của lớp đồ thị và khả năng phát hiện quan hệ ẩn chưa được đo.
