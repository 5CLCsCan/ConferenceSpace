...
Phụ lục này cung cấp thông tin kỹ thuật để tái lập môi trường phát triển và đối chiếu quy trình triển khai đã trình bày trong Chương~\ref{Chapter3}. Nội dung được tổ chức theo trình tự vận hành, từ chuẩn bị môi trường cục bộ, cấu hình các dịch vụ, đóng gói container đến phát hành image và cập nhật hệ thống chính thức. Các tên dịch vụ, cổng, biến cấu hình và lệnh được giữ theo hai tài liệu nguồn của Chương~3 để bảo đảm khả năng truy vết.

\section{Phạm vi và nguyên tắc triển khai}
\label{sec:appendix-b-scope}

Hệ thống được vận hành theo hai chế độ. Môi trường phát triển cục bộ ưu tiên khả năng kiểm tra độc lập từng thành phần, trong khi môi trường chính thức ưu tiên tính lặp lại của bản phát hành, cô lập mạng và bảo vệ bí mật. Hai chế độ dùng cùng các thành phần nền tảng nhưng khác cách khởi tạo và cung cấp cấu hình.
...
Triển khai chính thức & Phát hành phiên bản có thể tái tạo và thay thế nhất quán & Caddy, frontend, backend, dịch vụ AI, tác vụ migration, PostgreSQL, Redis và Neo4j chạy bằng Docker Compose; ba thành phần ứng dụng dùng image phát hành từ GitHub Container Registry \\ \hline
\end{tabularx}
\end{table}

Các nguyên tắc vận hành gồm: tách bí mật khỏi mã nguồn và image; chỉ công bố các cổng cần thiết; đặt các dịch vụ nội bộ trong cùng mạng Docker; duy trì dữ liệu bằng volume; kiểm tra trạng thái dịch vụ trước khi xác nhận triển khai; và dùng định danh phiên bản bất biến thay cho việc phụ thuộc duy nhất vào nhãn mới nhất.
...
Máy phát triển cần có Docker cùng Docker Compose, Go phiên bản 1.24, Node.js phiên bản 20 trở lên và npm. Docker cung cấp PostgreSQL, Redis và Neo4j; Go chạy backend; Node.js và npm chạy frontend. Cách tách này cho phép lập trình viên khởi động lại mã ứng dụng mà không phải tái tạo các dịch vụ dữ liệu.

\subsection{Quy trình khởi động môi trường}

Từ thư mục \path{backend}, Makefile tự động hóa trình tự khởi tạo kho dữ liệu, migration, đồ thị, dịch vụ AI, tài liệu API và backend. Target chính của quy trình là:
...
Dịch vụ AI có thể chạy trong Docker Compose hoặc chạy độc lập để kiểm tra điểm tích hợp.
...
Backend health & \url{http://localhost:8080/health} & 8080 & Phản hồi kiểm tra sức khỏe thành công \\ \hline
...
Môi trường chỉ được xem là sẵn sàng sau khi hoàn tất bốn bước kiểm tra. Thứ nhất, các container PostgreSQL, Redis và Neo4j phải ở trạng thái hoạt động. Thứ hai, backend phải hoàn tất kết nối cơ sở dữ liệu và migration mà không có lỗi. Thứ ba, điểm kiểm tra sức khỏe phải trả phản hồi thành công. Thứ tư, frontend phải tải được và gửi yêu cầu qua backend. Trình tự này giúp phân biệt lỗi hạ tầng, lỗi migration và lỗi tích hợp giao diện thay vì gộp chúng thành một lỗi khởi động chung.
...
Backend đọc cấu hình từ tệp \path{backend/.env} trong môi trường phát triển. Bảng~\ref{tab:appendix-b-backend-env} giữ nguyên các tên biến cấu hình được mô tả trong nguồn, đồng thời nêu rõ phạm vi trách nhiệm của từng nhóm.
...
\CodeBreak{FROM\_EMAIL} & Địa chỉ người gửi thư điện tử \\ \hline
\CodeBreak{EMAIL\_BASE\_URL} & Địa chỉ cơ sở dùng trong liên kết gửi qua thư điện tử \\ \hline
\end{longtable}

Các bí mật như mật khẩu cơ sở dữ liệu, khóa JWT, khóa dịch vụ AI và thông tin SMTP không được ghi trực tiếp vào mã nguồn, Dockerfile hoặc image. Mỗi môi trường phải cung cấp giá trị riêng thông qua tệp cấu hình không được theo dõi bởi Git hoặc thông qua cơ chế quản lý bí mật của nền tảng triển khai.

\subsection{Cấu hình dịch vụ AI}

Dịch vụ AI đọc tệp \path{ai-service/.env} bằng \CodeBreak{pydantic-settings}. Nhóm cấu hình nhà cung cấp phải hợp lệ theo một trong hai lựa chọn: OpenRouter với \CodeBreak{OPENROUTER\_API\_KEY} và \CodeBreak{AGENT\_MODEL}, hoặc nhà cung cấp tương thích với API của OpenAI với \CodeBreak{OPENAI\_API\_KEY}, \CodeBreak{OPENAI\_BASE\_URL} và \CodeBreak{OPENAI\_MODEL}. Nguồn quy định giá trị mặc định \CodeBreak{AGENT\_MODEL=openrouter/google/gemini-3.1-flash-lite}, \CodeBreak{OPENAI\_MODEL=gemini-3.1-flash-lite} và \CodeBreak{LLM\_REQUEST\_TIMEOUT\_SECONDS=60}; các khóa truy cập không được ghi giá trị trong báo cáo.
...
Môi trường chính thức sử dụng máy chủ riêng ảo chạy Ubuntu Server 22.04 LTS hoặc mới hơn. Cấu hình tối thiểu gồm 2 vCPU, 4 GB RAM và 30 GB dung lượng lưu trữ; cấu hình đề xuất khi tải tăng là ít nhất 4 vCPU và 8 GB RAM. Hệ thống gồm Caddy, frontend, backend, tác vụ migration, dịch vụ AI, PostgreSQL, Redis và Neo4j. Caddy là điểm vào duy nhất cho lưu lượng HTTP và HTTPS. Frontend, backend và dịch vụ AI không công bố trực tiếp các cổng ứng dụng ra Internet; PostgreSQL, Redis và Neo4j chỉ phục vụ các thành phần nội bộ và duy trì dữ liệu bằng volume.
...
\begin{tikzpicture}[>=Latex,
  ...
  \node[public] (caddy) at (0,-2.2) {Caddy\\TLS và reverse proxy};
  \node[app] (frontend) at (-4.0,-5.0) {Frontend\\Next.js};
  \node[app] (backend) at (0,-5.0) {Backend\\Go và Gin};
  \node[app] (ai) at (4.0,-5.0) {Dịch vụ AI\\FastAPI};
  \node[data] (postgres) at (-4.0,-8.3) {PostgreSQL};
  \node[data] (redis) at (0,-8.3) {Redis};
  \node[data] (neo4j) at (4.0,-8.3) {Neo4j};
  \draw[-{Latex},thick] (client) -- node[right] {HTTPS} (caddy);
  \draw[-{Latex},thick] (caddy) -- node[left] {giao diện} (frontend);
  \draw[-{Latex},thick] (caddy) -- node[right] {WebSocket} (backend);
  \draw[-{Latex},thick] (frontend) -- node[above] {API} (backend);
  \draw[-{Latex},thick] (frontend) -- (ai);
  \draw[-{Latex},thick] (backend) -- (ai);
  \draw[-{Latex},thick] (backend) -- (postgres);
  \draw[-{Latex},thick] (backend) -- (neo4j);
  \draw[-{Latex},thick] (ai) -- (postgres);
...
\hline
Dịch vụ & Trách nhiệm & Phạm vi truy cập & Dữ liệu bền vững \\ \hline
\endfirsthead
\hline
Dịch vụ & Trách nhiệm & Phạm vi truy cập & Dữ liệu bền vững \\ \hline
\endhead
Caddy & Kết thúc TLS, chuyển tiếp giao diện và WebSocket & Công khai cổng 80 và 443 & Chứng chỉ và dữ liệu Caddy \\ \hline
Frontend & Cung cấp giao diện Next.js, chuyển tiếp API và hội thoại & Nội bộ qua Caddy & Không lưu dữ liệu nghiệp vụ \\ \hline
Backend & Cung cấp API, xác thực, nghiệp vụ và tích hợp dịch vụ & Nội bộ qua frontend hoặc Caddy & Sử dụng volume tệp tải lên \\ \hline
...
PostgreSQL & Lưu dữ liệu quan hệ của hệ thống & Chỉ trong mạng Docker & Volume PostgreSQL \\ \hline
...
\section{Docker Compose chính thức}
\label{sec:appendix-b-compose}

\subsection{Cấu trúc dịch vụ}

Tệp \path{deployment/docker-compose.prod.yml} mô tả toàn bộ hệ thống chính thức. Frontend, backend và dịch vụ AI nhận image lần lượt từ \CodeBreak{FRONTEND\_IMAGE}, \CodeBreak{BACKEND\_IMAGE} và \CodeBreak{AI\_SERVICE\_IMAGE}; các dịch vụ dữ liệu dùng image nền tảng cố định theo phiên bản. Tác vụ \CodeBreak{backend-migrate} dùng image backend để áp dụng migration trước khi các container ứng dụng được cập nhật.

\begin{longtable}{|T{0.19\textwidth}|T{0.31\textwidth}|T{0.18\textwidth}|T{0.22\textwidth}|}
\caption{Image và cấu phần đóng gói của các dịch vụ}\label{tab:appendix-b-images}\\
\hline
...
Redis & redis & 7-alpine & Kho dữ liệu tạm thời \\ \hline
Neo4j & neo4j & 5.15-community & Cơ sở dữ liệu đồ thị \\ \hline
\end{longtable}

Ký hiệu \textit{[owner]} trong tên image được thay bằng chủ sở hữu kho mã khi pipeline phát hành chạy. Nhãn theo định danh commit là tham chiếu bất biến dùng để truy vết phiên bản; nhãn \textit{latest} chỉ biểu thị bản phát hành mới nhất và không thay thế định danh commit trong hồ sơ triển khai.

\subsection{Cổng, mạng và quan hệ phụ thuộc}

Chỉ Caddy công bố cổng 80 và 443. Các cổng 3000 của frontend, 8080 của backend, 8090 của dịch vụ AI, 5432 của PostgreSQL, 6379 của Redis, 7474 và 7687 của Neo4j chỉ dùng trong mạng nội bộ. Frontend phụ thuộc vào backend và dịch vụ AI; backend phụ thuộc vào PostgreSQL, Neo4j và dịch vụ AI; dịch vụ AI phụ thuộc vào PostgreSQL và Redis.

Cấu hình chính thức tách mạng \textit{app} và \textit{data}. Mạng \textit{data} dùng thuộc tính \textit{internal: true}; PostgreSQL và Redis chỉ tham gia mạng này. Backend và dịch vụ AI tham gia cả hai mạng; Neo4j cũng tham gia hai mạng trong cấu hình hiện tại; Caddy chỉ tham gia mạng \textit{app}. Việc tách mạng giảm số dịch vụ dữ liệu có thể bị truy cập trực tiếp từ Internet nhưng không được xem là bằng chứng bảo mật toàn diện.

\subsection{Volume và tính bền vững dữ liệu}
...
Thành phần & Nhóm dữ liệu & Mục đích \\ \hline
\endhead
PostgreSQL & Dữ liệu quan hệ & Duy trì người dùng, hội nghị, bài nộp, phân công, phản biện và quyết định \\ \hline
Redis & Dữ liệu Redis & Duy trì dữ liệu khi cấu hình vận hành yêu cầu khả năng phục hồi sau khi thay container \\ \hline
Neo4j & Dữ liệu đồ thị & Duy trì nút, quan hệ đồng tác giả và dữ liệu phục vụ phát hiện xung đột lợi ích \\ \hline
...
Các volume được khai báo gồm \CodeBreak{caddy\_data}, \CodeBreak{caddy\_config}, \CodeBreak{postgres\_data}, \CodeBreak{redis\_data}, \CodeBreak{neo4j\_data}, \CodeBreak{neo4j\_logs}, \CodeBreak{neo4j\_plugins} và \CodeBreak{uploads\_data}. Việc xóa hoặc tái tạo container không được đồng nghĩa với xóa volume. Các thao tác ảnh hưởng volume phải được xem là thao tác dữ liệu, cần có bản sao lưu và xác nhận riêng thay vì được gộp vào quy trình cập nhật ứng dụng.
...
Tệp \path{frontend/Dockerfile} dùng quy trình nhiều giai đoạn. Giai đoạn phụ thuộc cài đặt các gói Node.js; giai đoạn xây dựng tạo bản Next.js cho môi trường chính thức; giai đoạn chạy chỉ chứa các tệp cần thiết. Cách tách này giảm kích thước image và tránh đưa toàn bộ công cụ xây dựng vào container đang phục vụ người dùng.
...
Giai đoạn & Đầu vào chính & Kết quả \\ \hline
\endhead
Builder & Go 1.24 Alpine, mã nguồn và module & Tạo Swagger, biên dịch tệp thực thi Linux tĩnh và cài công cụ \textit{migrate} \\ \hline
Runner & Tệp thực thi, migration, công cụ \textit{migrate} và chứng chỉ CA & Container Alpine chạy backend trên cổng 8080 \\ \hline
\end{longtable}

Image backend không chứa mật khẩu cơ sở dữ liệu, khóa JWT, khóa nhà cung cấp mô hình hoặc thông tin SMTP. Các giá trị này được cung cấp khi container khởi động. Việc thay bí mật vì vậy không yêu cầu xây dựng lại image ứng dụng.
...
Việc chạy migration cùng lệnh khởi động bảo đảm lược đồ của dịch vụ AI được cập nhật trước khi nhận yêu cầu. Điểm kiểm tra \path{/health} và \path{/ready} được dùng để phân biệt tiến trình đã chạy với trạng thái sẵn sàng phục vụ.

\subsection{Kiểm tra image trước khi phát hành}

Mỗi image phải xây dựng từ đúng Dockerfile và mang nhãn định danh commit để có thể truy ngược về mã nguồn. Các điểm kiểm tra sức khỏe của backend và dịch vụ AI cung cấp tiêu chí kỹ thuật để kiểm tra container sau khi khởi động. Tuy nhiên, workflow hiện hành chưa bao gồm đầy đủ bước kiểm tra chất lượng bắt buộc hoặc kiểm tra hoạt động cơ bản sau triển khai; vì vậy, nguồn không đủ để khẳng định các kiểm tra này là cổng bắt buộc trước khi cập nhật nhãn latest.
...
Deployment & Cập nhật ba biến image, pull, chạy \CodeBreak{backend-migrate} và khởi động Compose & Container mới đạt điểm kiểm tra sức khỏe \\ \hline
\end{longtable}
...
Định danh commit là khóa truy vết giữa image, mã nguồn và lần chạy workflow. Khi cần quay lại phiên bản trước, hệ thống chọn lại image theo định danh commit thay vì suy đoán nội dung của nhãn \textit{latest}.
...
\caption{Phân loại và biện pháp bảo vệ cấu hình}\label{tab:appendix-b-secret-controls}\\
\hline
...
Khóa dịch vụ bên ngoài & \CodeBreak{GEMINI\_API\_KEY}, \CodeBreak{OPENROUTER\_API\_KEY} và nhóm biến nhà cung cấp AI & Lưu trong GitHub Secrets hoặc kho bí mật của máy chủ; giới hạn phạm vi quyền; thay khóa khi nghi ngờ lộ \\ \hline
Thông tin thư điện tử & \CodeBreak{SMTP\_USERNAME}, \CodeBreak{SMTP\_PASSWORD} & Chỉ cung cấp cho backend; không đưa vào frontend hoặc image \\ \hline
...
2 & PostgreSQL, Redis và Neo4j khởi động và đạt trạng thái sẵn sàng & Trạng thái container và healthcheck \\ \hline
3 & Backend hoàn tất migration và trả phản hồi tại điểm kiểm tra sức khỏe & Nhật ký khởi động và phản hồi health \\ \hline
...
11 & Quy trình quay lại phiên bản đã được kiểm tra bằng image có định danh bất biến & Định danh image ổn định và biên bản kiểm tra \\ \hline
\end{longtable}

Phụ lục B kết thúc tại mức cần thiết để tái lập và kiểm tra môi trường: thành phần, phiên bản, cấu hình, image, định tuyến, dữ liệu bền vững, quy trình phát hành và điểm nghiệm thu. Các kết quả đánh giá hiệu năng và chất lượng hệ thống không thuộc phạm vi phụ lục này; chúng được trình bày cùng Chương~4 và Phụ lục~C để tránh trộn lẫn quyết định triển khai với bằng chứng đánh giá.