...
\hline
\TableHeader{Chế độ} & \TableHeader{Mục tiêu} & \TableHeader{Cách vận hành} \\ \hline
...
Từ thư mục \path{backend}, Makefile tự động hóa việc khởi động các dịch vụ dữ liệu, chạy migration, khởi tạo Neo4j, tạo lại tài liệu Swagger và chạy backend. Đoạn mã~\ref{lst:appendix-b-local-make-dev} trích các tác vụ chính của quy trình.
...
Frontend được khởi động trong thư mục \path{frontend} theo các lệnh ở Đoạn mã~\ref{lst:appendix-b-local-frontend}.
...
Dịch vụ AI có thể chạy cùng Docker Compose hoặc chạy riêng khi cần kiểm tra trực tiếp các điểm tích hợp. Đoạn mã~\ref{lst:appendix-b-local-ai} trình bày trình tự cài đặt, cập nhật lược đồ và khởi động dịch vụ khi chạy riêng.
...
Bảng~\ref{tab:appendix-b-local-endpoints} liệt kê các điểm truy cập và tiêu chí kiểm tra tương ứng trong môi trường cục bộ.

\begingroup
\setlength{\tabcolsep}{4pt}
\begin{longtable}{|T{0.18\textwidth}|T{0.32\textwidth}|T{0.10\textwidth}|T{0.30\textwidth}|}
\caption{Điểm truy cập trong môi trường phát triển cục bộ}\label{tab:appendix-b-local-endpoints}\\
\hline
\TableHeader{Thành phần} & \TableHeader{Điểm truy cập} & \TableHeader{Cổng} & \TableHeader{Tiêu chí kiểm tra} \\ \hline
\endfirsthead
\hline
\TableHeader{Thành phần} & \TableHeader{Điểm truy cập} & \TableHeader{Cổng} & \TableHeader{Tiêu chí kiểm tra} \\ \hline
\endhead
...
\begin{longtable}{|T{0.40\textwidth}|T{0.50\textwidth}|}
\caption{Biến cấu hình của backend}\label{tab:appendix-b-backend-env}\\
\hline
\TableHeader{Biến cấu hình} & \TableHeader{Mục đích} \\ \hline
\endfirsthead
\hline
\TableHeader{Biến cấu hình} & \TableHeader{Mục đích} \\ \hline
\endhead
...
Frontend đọc cấu hình từ tệp \path{frontend/.env.local} trong môi trường phát triển. Các biến công khai chỉ được dùng cho giá trị có thể xuất hiện trong trình duyệt; khóa bí mật không được đặt trong biến có tiền tố công khai. Bảng~\ref{tab:appendix-b-frontend-env} mô tả các biến được dùng tại phía trình duyệt và phía máy chủ của frontend.
...
Môi trường chính thức được triển khai trên máy chủ riêng ảo chạy Ubuntu Server 22.04 LTS hoặc mới hơn. Cấu hình tối thiểu gồm 2 vCPU, 4 GB RAM và 30 GB dung lượng lưu trữ; cấu hình đề xuất khi tải tăng là ít nhất 4 vCPU và 8 GB RAM. Hệ thống gồm Caddy, frontend, backend, tác vụ migration, dịch vụ AI, PostgreSQL, Redis và Neo4j. Caddy đóng vai trò là điểm vào công khai cho lưu lượng HTTP và HTTPS. Frontend, backend và dịch vụ AI không mở trực tiếp các cổng ứng dụng ra Internet; PostgreSQL, Redis và Neo4j chỉ phục vụ các thành phần nội bộ và duy trì dữ liệu bằng volume. Hình~\ref{fig:appendix-b-production-topology} trình bày quan hệ giữa các thành phần; Bảng~\ref{tab:appendix-b-production-services} nêu trách nhiệm và phạm vi truy cập của từng dịch vụ.
...
\begin{tikzpicture}[>=Latex,
  ...
  \node[public] (client) at (0,0.5) {Trình duyệt người dùng};
  \node[public] (caddy) at (0,-1.9) {Caddy\\TLS và reverse proxy};
  \node[app] (frontend) at (-4.0,-4.3) {Frontend\\Next.js};
  \node[app] (backend) at (3.0,-4.3) {Backend\\Go và Gin};
  \node[app] (ai) at (0,-6) {Dịch vụ AI\\FastAPI};
  \node[data] (neo4j) at (5.4,-6.7) {Neo4j};
  \node[data] (redis) at (-2.7,-8.5) {Redis};
  \node[data] (postgres) at (2.7,-8.5) {PostgreSQL};
...
\begingroup
\setlength{\tabcolsep}{4pt}
\begin{longtable}{|T{0.15\textwidth}|T{0.35\textwidth}|T{0.20\textwidth}|T{0.20\textwidth}|}
\caption{Trách nhiệm của các dịch vụ trong môi trường chính thức}\label{tab:appendix-b-production-services}\\
\hline
\TableHeader{Dịch vụ} & \TableHeader{Trách nhiệm} & \TableHeader{Phạm vi truy cập} & \TableHeader{Dữ liệu cần duy trì} \\ \hline
\endfirsthead
\hline
\TableHeader{Dịch vụ} & \TableHeader{Trách nhiệm} & \TableHeader{Phạm vi truy cập} & \TableHeader{Dữ liệu cần duy trì} \\ \hline
\endhead
...
Dịch vụ AI & Thực thi sáu luồng công việc AI, migration Alembic và truy vấn backend có kiểm soát & Nội bộ qua frontend và backend & Dùng PostgreSQL và Redis \\ \hline
...
Tệp \path{deployment/docker-compose.prod.yml} mô tả các dịch vụ cần chạy trong môi trường chính thức. Frontend, backend và dịch vụ AI nhận image lần lượt từ \CodeBreak{FRONTEND\_IMAGE}, \CodeBreak{BACKEND\_IMAGE} và \CodeBreak{AI\_SERVICE\_IMAGE}; các dịch vụ dữ liệu dùng image nền tảng cố định theo phiên bản. Tác vụ \CodeBreak{backend-migrate} dùng image backend để áp dụng migration trước khi các container ứng dụng được cập nhật. Bảng~\ref{tab:appendix-b-images} đối chiếu image, phiên bản và thành phần đóng gói của từng dịch vụ.

\begingroup
\setlength{\tabcolsep}{4pt}
\begin{longtable}{|T{0.16\textwidth}|T{0.27\textwidth}|T{0.17\textwidth}|T{0.30\textwidth}|}
\caption{Image và thành phần đóng gói của các dịch vụ}\label{tab:appendix-b-images}\\
\hline
\TableHeader{Dịch vụ} & \TableHeader{Image/tệp đóng gói} & \TableHeader{Phiên bản} & \TableHeader{Ghi chú} \\ \hline
\endfirsthead
\hline
\TableHeader{Dịch vụ} & \TableHeader{Image/tệp đóng gói} & \TableHeader{Phiên bản} & \TableHeader{Ghi chú} \\ \hline
\endhead
Caddy & caddy & 2-alpine & Reverse proxy và TLS \\ \hline
Frontend & \CodeBreak{ghcr.io/[owner]/conferencespace-frontend} & Theo commit SHA, latest & Node 22 Alpine; xây dựng từ \path{frontend/Dockerfile} \\ \hline
Backend & \CodeBreak{ghcr.io/[owner]/conferencespace-backend} & Theo commit SHA, latest & Go 1.24 Alpine; xây dựng từ \path{backend/Dockerfile} \\ \hline
Dịch vụ AI & \CodeBreak{ghcr.io/[owner]/conferencespace-ai-service} & Theo commit SHA, latest & Python 3.12 slim; xây dựng từ \path{ai-service/Dockerfile} \\ \hline
...
Bảng~\ref{tab:appendix-b-volumes} liệt kê các nhóm dữ liệu phải được duy trì độc lập với vòng đời của container.

\begin{longtable}{|T{0.20\textwidth}|T{0.20\textwidth}|T{0.50\textwidth}|}
\caption{Dữ liệu cần duy trì ngoài vòng đời container}\label{tab:appendix-b-volumes}\\
\hline
\TableHeader{Thành phần} & \TableHeader{Nhóm dữ liệu} & \TableHeader{Mục đích} \\ \hline
\endfirsthead
\hline
\TableHeader{Thành phần} & \TableHeader{Nhóm dữ liệu} & \TableHeader{Mục đích} \\ \hline
\endhead
...
Tệp \path{frontend/Dockerfile} dùng quy trình nhiều giai đoạn. Giai đoạn phụ thuộc cài đặt các gói Node.js; giai đoạn xây dựng tạo bản Next.js cho môi trường chính thức; giai đoạn chạy chỉ chứa các tệp cần thiết. Cách chia giai đoạn này giúp giảm kích thước image khi chạy và tránh đưa các công cụ xây dựng vào container phục vụ người dùng. Bảng~\ref{tab:appendix-b-frontend-dockerfile} tóm tắt đầu vào và kết quả của từng giai đoạn.

\begin{longtable}{|T{0.20\textwidth}|T{0.35\textwidth}|T{0.35\textwidth}|}
\caption{Các giai đoạn đóng gói frontend}\label{tab:appendix-b-frontend-dockerfile}\\
\hline
\TableHeader{Giai đoạn} & \TableHeader{Đầu vào chính} & \TableHeader{Kết quả} \\ \hline
\endfirsthead
\hline
\TableHeader{Giai đoạn} & \TableHeader{Đầu vào chính} & \TableHeader{Kết quả} \\ \hline
\endhead
...
Tệp \path{backend/Dockerfile} cũng dùng quy trình nhiều giai đoạn. Giai đoạn xây dựng tải module Go và biên dịch tệp thực thi; giai đoạn chạy dùng image nhỏ hơn, chứa tệp thực thi, chứng chỉ cần cho kết nối HTTPS và các tài nguyên cần thiết khi chạy. Bảng~\ref{tab:appendix-b-backend-dockerfile} tóm tắt hai giai đoạn đóng gói này.

\begin{longtable}{|T{0.15\textwidth}|T{0.35\textwidth}|T{0.40\textwidth}|}
...
Tệp \path{ai-service/Dockerfile} sử dụng Python 3.12 slim, cài \textit{libmagic1}, \textit{curl} và Poetry 1.8.5, sau đó cài nhóm phụ thuộc chính mà không tạo môi trường ảo trong container. Đoạn mã~\ref{lst:appendix-b-ai-dockerfile} cho thấy lệnh khởi động thực hiện migration Alembic trước khi chạy Uvicorn.
...
Mỗi image phải xây dựng từ đúng Dockerfile và mang nhãn định danh commit để có thể truy ngược về mã nguồn. Các điểm kiểm tra trạng thái của backend và dịch vụ AI cung cấp tiêu chí kỹ thuật để kiểm tra container sau khi khởi động. Tuy nhiên, luồng công việc (workflow) CI/CD hiện hành chưa bao gồm đầy đủ bước kiểm tra chất lượng bắt buộc hoặc kiểm tra hoạt động cơ bản sau triển khai; vì vậy, nguồn không đủ để khẳng định các kiểm tra này là cổng bắt buộc trước khi cập nhật nhãn latest.

\section{Caddy, TLS và định tuyến}
\label{sec:appendix-b-caddy}

Caddy nhận lưu lượng công khai, tự động quản lý TLS và chuyển tiếp yêu cầu đến dịch vụ phù hợp. Lưu lượng WebSocket được chuyển trực tiếp đến backend; các yêu cầu còn lại được chuyển đến frontend, nơi các route phía máy chủ chuyển tiếp yêu cầu API khi cần. Caddy là thành phần duy nhất cần tiếp xúc trực tiếp với Internet, nhờ đó giảm số cổng phải bảo vệ tại máy chủ. Bảng~\ref{tab:appendix-b-caddy-routing} tóm tắt ranh giới định tuyến này.
...
\hline
\TableHeader{Nhóm yêu cầu} & \TableHeader{Đích đến} & \TableHeader{Trách nhiệm} \\ \hline
...
\clearpage
Cấu hình Caddy hiện tại dùng hai tên miền: \path{conference-space.com} và \path{www.conference-space.com}. Các quy tắc định tuyến được trình bày trong Đoạn mã~\ref{lst:appendix-b-caddyfile}.
...
\subsection{Phạm vi luồng công việc CI/CD}

Luồng công việc trong \path{.github/workflows/deploy.yml} không xây dựng image trên máy chủ. GitHub Actions xây dựng và đẩy song song ba image frontend, backend và dịch vụ AI lên GHCR; máy chủ VPS chỉ lấy image, cập nhật tệp triển khai, chạy migration và thay container. Luồng công việc được kích hoạt khi commit vào nhánh \textit{main} hoặc khi người vận hành yêu cầu chạy thủ công. ng~\ref{tab:appendix-b-cicd-stages} trình bày các giai đoạn và điều kiện hoàn tất tương ứng.

\begin{longtable}{|T{0.20\textwidth}|T{0.40\textwidth}|T{0.35\textwidth}|}
\caption{Các giai đoạn của quy trình CI/CD}\label{tab:appendix-b-cicd-stages}\\
\hline
\TableHeader{Giai đoạn} & \TableHeader{Kiểm tra chính} & \TableHeader{Điều kiện hoàn tất} \\ \hline
\endfirsthead
\hline
\TableHeader{Giai đoạn} & \TableHeader{Kiểm tra chính} & \TableHeader{Điều kiện hoàn tất} \\ \hline
\endhead
...
Ba image được phát hành theo các mẫu trong Đoạn mã~\ref{lst:appendix-b-ghcr-images}.
...
Định danh commit là điểm nối để truy vết giữa image, mã nguồn và lần chạy luồng công việc. Khi cần quay lại phiên bản trước, người vận hành có thể chọn lại image theo định danh commit thay vì phụ thuộc vào nhãn \textit{latest}.

\subsection{Cập nhật hệ thống chính thức}

Tác vụ triển khai cập nhật ba biến image trong \path{.env.production}, đăng nhập GHCR, lấy image, chạy migration bằng dịch vụ \CodeBreak{backend-migrate}, rồi khởi động toàn bộ hệ thống theo Đoạn mã~\ref{lst:appendix-b-production-update}.
...
Bảng~\ref{tab:appendix-b-secret-controls} phân loại các nhóm cấu hình và biện pháp bảo vệ tương ứng.

\begin{longtable}{|T{0.23\textwidth}|T{0.30\textwidth}|T{0.40\textwidth}|}
\caption{Phân loại cấu hình và biện pháp bảo vệ}\label{tab:appendix-b-secret-controls}\\
\hline
\TableHeader{Nhóm cấu hình} & \TableHeader{Ví dụ} & \TableHeader{Biện pháp} \\ \hline
\endfirsthead
\hline
\TableHeader{Nhóm cấu hình} & \TableHeader{Ví dụ} & \TableHeader{Biện pháp} \\ \hline
\endhead
...
9 & Luồng công việc xây dựng ba image trước khi phát hành và triển khai & Kết quả lần chạy GitHub Actions và GHCR \\ \hline
...
Phụ lục B cung cấp thông tin tái lập môi trường, gồm thành phần, phiên bản, cấu hình, image, định tuyến, dữ liệu cần duy trì, quy trình phát hành và điểm nghiệm thu. Kết quả đánh giá được trình bày tại Chương~\ref{Chapter4} và Phụ lục~\ref{AppendixC}.
