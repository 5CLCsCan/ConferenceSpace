...
Giai đoạn **CD (Continuous Deployment):** Sau khi CI thành công, pipeline build song song ba Docker image (frontend, backend, ai-service), tag theo Git commit hash và push lên GitHub Container Registry (GHCR). Sau đó, pipeline SSH vào VPS production, copy file cấu hình (`docker-compose.prod.yml`, `Caddyfile`), chạy database migration qua container `backend-migrate` và thực thi `docker compose up -d --remove-orphans` để cập nhật container với image mới. Toàn bộ quá trình từ push code đến service cập nhật trên production mất khoảng 5–8 phút.
