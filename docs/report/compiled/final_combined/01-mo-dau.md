...
- **Thiết kế các workflow AI có kiểm soát**, luôn giữ vai trò hỗ trợ (Submission Autofill cho form nộp bài, Submission Gating cho kiểm tra sơ bộ bản thảo, tóm lược cho reviewer, rà soát chất lượng phản biện, tổng hợp thông tin cho chair), không thay thế quyết định học thuật cuối cùng của con người — theo đúng nguyên tắc "AI as assistant, not arbiter" được khuyến nghị trong các nghiên cứu gần đây về ứng dụng LLM vào peer review [8][13].
...
- Sáu workflow AI hỗ trợ theo vai trò: Submission Autofill (tự động điền metadata từ bản thảo PDF và gợi ý track trong cùng workflow), Submission Gating (kiểm tra sơ bộ bản thảo ở cổng nộp bài), Reviewer Initial Analysis (tóm lược hỗ trợ reviewer), Review Quality Auditor (rà soát chất lượng phản biện), Chair Decision Copilot (hỗ trợ tổng hợp thông tin cho chair) và Chatbot Agent (trợ lý truy vấn ngữ cảnh hệ thống theo phân quyền). Việc dùng LLM cho các nhiệm vụ tóm lược và trích xuất metadata từ PDF có cơ sở kỹ thuật: nghiên cứu MOLE (2025) cho thấy LLM có thể trích xuất metadata từ bài báo khoa học ở mức đáng tin cậy khi kết hợp bước xác thực đầu ra [16].
