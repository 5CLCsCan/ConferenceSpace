...
Hướng cải tiến trực tiếp là đưa cơ chế hậu kiểm trong benchmark vào runtime. Cụ thể, hệ thống có thể trích các claim chính từ finding, truy xuất các đoạn evidence liên quan trong review và submission, dùng NLI để kiểm tra quan hệ claim-evidence, sau đó dùng cơ chế xếp hạng/hợp nhất bằng chứng kiểu RRF để ưu tiên các finding có nhiều nguồn hỗ trợ độc lập. Phần NLI của hướng này đã được dùng ở mức benchmark hậu kiểm trong Chương 4 thông qua TCA để đo truthfulness và grounded-valid rate của Review Quality Auditor; RRF là lớp bổ sung hợp lý nếu muốn chuyển hậu kiểm từ offline benchmark sang runtime. Nếu được triển khai vào runtime, các finding không đủ groundedness có thể bị hạ từ `block` xuống `warn`, hoặc được yêu cầu Chair/reviewer xác nhận trước khi hệ thống chặn gửi chính thức.
